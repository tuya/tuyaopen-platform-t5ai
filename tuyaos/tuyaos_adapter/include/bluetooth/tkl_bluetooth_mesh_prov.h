 ...
 */
OPERATE_RET tkl_ble_mesh_prov_scan(UCHAR_T enable, uint32_t timeout);
 ...
 *          ERROR: failure
 */
OPERATE_RET tkl_ble_mesh_device_update_iv_info(uint32_t iv_index, UCHAR_T flags);
 ...
 *          ERROR
 * */
OPERATE_RET tkl_ble_mesh_vendor_command_control(TKL_MESH_DEVICE_INFO_T device, USHORT_T opcode, void *args);


OPERATE_RET tkl_ble_mesh_dfu_init(const TKL_MESH_DFU_IO_T *p_dfu_io);

OPERATE_RET tkl_ble_mesh_dfu_control(TKL_MESH_DFU_OPERATION_E operation, void *args, USHORT_T args_len);
