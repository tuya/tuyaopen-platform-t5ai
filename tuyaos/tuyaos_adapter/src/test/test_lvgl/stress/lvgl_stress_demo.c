...
static void obj_test_task_cb(lv_timer_t * tmr)
{
    ...
    switch(state) {
        ...
        case 31:
            ...
            state = -2;
            // cnt++;
            // bk_printf("xxxxxxxxxxxxxxxx: %d\r\n", cnt);
            // tkl_lvgl_msg_sync((uint8_t *)&cnt, sizeof(cnt));
            break;
        default:
            break;
    }
...
void tuya_lvgl_stress_pause(void)
{
    lv_timer_pause(tuya_gui_timer);
}

void tuya_lvgl_stress_resume(void)
{
    lv_timer_resume(tuya_gui_timer);
}
