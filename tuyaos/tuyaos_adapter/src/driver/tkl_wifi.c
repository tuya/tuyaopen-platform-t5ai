...
extern uint8_t ready_to_deepsleep;
...
{
    if (ready_to_deepsleep || !wifi_event_cb)
        return;
...
OPERATE_RET tkl_wifi_set_country_code(const COUNTRY_CODE_E ccode)
{
    ...
    if (ret != OPRT_OK) {
        tkl_log_output("set country err! %d\r\n", ccode);
    }
