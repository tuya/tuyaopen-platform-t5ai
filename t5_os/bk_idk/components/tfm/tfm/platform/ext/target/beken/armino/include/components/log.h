...
#define BK_LOGD(tag, format, ...) if (efuse_is_info_log_enabled()) _OS_PRINTF(BK_LOG_FORMAT(D, format), rtos_get_time(), tag, ## __VA_ARGS__)
#else
#define BK_LOGD(tag, format, ...) do { (void)sizeof(tag, format, ## __VA_ARGS__); } while (0)
#endif

#if (LOG_LEVEL >= BK_LOG_VERBOSE)
#define BK_LOGV(tag, format, ...) if (efuse_is_info_log_enabled()) _OS_PRINTF(BK_LOG_FORMAT(V, format), rtos_get_time(), tag, ## __VA_ARGS__)
#else
