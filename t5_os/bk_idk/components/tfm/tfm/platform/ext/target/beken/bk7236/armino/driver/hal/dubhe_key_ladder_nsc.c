                         ...
                         unsigned char *output )
{
	...
	in_temp  = malloc( length );
	out_temp = malloc( length );
	if( in_temp ==NULL || out_temp == NULL )
	{
		ret = MBEDTLS_ERR_AES_MALLOC;
		goto clearup;
	}
	memcpy( in_temp, input, length );
                         ...
                         unsigned char *output )
{
	...
	const unsigned char * input_temp = malloc(length);
	unsigned char * output_temp = malloc(length);
