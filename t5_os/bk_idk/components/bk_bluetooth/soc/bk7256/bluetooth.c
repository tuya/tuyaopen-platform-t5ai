...
static void phy_enable_rx_switch_wrapper(void)
{
#if CONFIG_WIFI_ENABLE
    extern void phy_enable_rx_switch(void);
...
void enable_debug_gpio_wrapper(void)
{
    ...
    gpio_dev_map(GPIO_14, GPIO_DEV_DEBUG8);//P14(CON3)
    gpio_dev_unmap(GPIO_15);
    gpio_dev_map(GPIO_15, GPIO_DEV_DEBUG9);//P15(CON3)
    gpio_dev_unmap(GPIO_16);
    gpio_dev_map(GPIO_16, GPIO_DEV_DEBUG10);//S14(H3)
    ...
    {
        bk_printf("%s, external 32k is in use, GPIO32 and GPIO33 can not be used to debug \r\n", __func__);
    }
...
static void coexist_check_large_signal_wrapper(uint8_t switch2bt)
{
#if CONFIG_WIFI_ENABLE
    extern void coexist_check_large_signal(uint8_t switch2bt);
    coexist_check_large_signal(switch2bt);
#endif
}
...
static struct bt_osi_funcs_t bt_osi_funcs = {
    ...
    ._phy_enable_rx_switch = phy_enable_rx_switch_wrapper,
