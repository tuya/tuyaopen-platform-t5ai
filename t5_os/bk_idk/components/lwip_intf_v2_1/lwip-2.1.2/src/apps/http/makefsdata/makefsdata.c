...
int main(int argc, char *argv[])
{
  ...
  LWIP_LOGD(NEWLINE " makefsdata - HTML to C source converter" NEWLINE);
  LWIP_LOGD("     by Jim Pettinato               - circa 2003 " NEWLINE);
  LWIP_LOGD("     extended by Simon Goldschmidt  - 2009 " NEWLINE NEWLINE);
    ...
    if (argv[i][0] == '-') {
      if (strstr(argv[i], "-svr:") == argv[i]) {
        ...
        if (checkSsiByFilelist(ssi_list_filename)) {
          LWIP_LOGD("Reading list of SSI files from \"%s\"\n", ssi_list_filename);
        } else {
          LWIP_LOGE("Failed to load list of SSI files from \"%s\"\n", ssi_list_filename);
        }
        ...
        if (colon) {
          if (colon[1] != 0) {
            ...
            if ((defl_level >= 0) && (defl_level <= 10)) {
              ...
              LWIP_LOGE("ERROR: deflate level must be [0..10]" NEWLINE);
              exit(0);
            }
          }
        }
        deflateNonSsiFiles = 1;
        LWIP_LOGI("Deflating all non-SSI files with level %d (but only if size is reduced)" NEWLINE, deflate_level);
#else
        LWIP_LOGI("WARNING: Deflate support is disabled\n");
#endif
      } else if (strstr(argv[i], "-x:") == argv[i]) {
        exclude_list = &argv[i][3];
        LWIP_LOGD("Excluding files with extensions %s" NEWLINE, exclude_list);
      } else if (strstr(argv[i], "-xc:") == argv[i]) {
        ncompress_list = &argv[i][4];
        LWIP_LOGD("Skipping compresion for files with extensions %s" NEWLINE, ncompress_list);
  ...
  if (!check_path(path, sizeof(path))) {
    LWIP_LOGE("Invalid path: \"%s\"." NEWLINE, path);
    exit(-1);
  }
  ...
  if (!CHDIR_SUCCEEDED(CHDIR(path))) {
    ...
    LWIP_LOGE(" Failed to open directory \"%s\"." NEWLINE NEWLINE, path);
    print_usage();
    exit(-1);
  }
  CHDIR(appPath);

  LWIP_LOGD("HTTP %sheader will %s statically included." NEWLINE,
         (includeHttpHeader ? (useHttp11 ? "1.1 " : "1.0 ") : ""),
         (includeHttpHeader ? "be" : "not be"));

  curSubdir[0] = '\0'; /* start off in web page's root directory - relative paths */
  LWIP_LOGD("  Processing all files in directory %s", path);
  if (processSubs) {
    LWIP_LOGI(" and subdirectories..." NEWLINE NEWLINE);
  } else {
    LWIP_LOGI("..." NEWLINE NEWLINE);
  }

  data_file = fopen("fsdata.tmp", "wb");
  if (data_file == NULL) {
    LWIP_LOGE("Failed to create file \"fsdata.tmp\"\n");
    exit(-1);
  }
  struct_file = fopen("fshdr.tmp", "wb");
  if (struct_file == NULL) {
    LWIP_LOGE("Failed to create file \"fshdr.tmp\"\n");
    fclose(data_file);
    exit(-1);
  }
  ...
  LWIP_LOGD(NEWLINE "Creating target file..." NEWLINE NEWLINE);
  concat_files("fsdata.tmp", "fshdr.tmp", targetfile);

  /* if succeeded, delete the temporary files */
  if (remove("fsdata.tmp") != 0) {
    LWIP_LOGE("Warning: failed to delete fsdata.tmp\n");
  }
  if (remove("fshdr.tmp") != 0) {
    LWIP_LOGE("Warning: failed to delete fshdr.tmp\n");
  }

  LWIP_LOGI(NEWLINE "Processed %d files - done." NEWLINE, filesProcessed);
#if MAKEFS_SUPPORT_DEFLATE
  if (deflateNonSsiFiles) {
    LWIP_LOGD("(Deflated total byte reduction: %d bytes -> %d bytes (%.02f%%)" NEWLINE,
...
static void copy_file(const char *filename_in, FILE *fout)
{
  ...
  if (fin == NULL) {
    LWIP_LOGE("Failed to open file \"%s\"\n", filename_in);
    exit(-1);
  }
      ...
      for (i = 0; i < dir.n_files; i++) {
        ...
        if (ret == 0) {
          ...
          if (freelen > 0) {
            ...
            LWIP_LOGD("processing subdirectory %s/..." NEWLINE, curSubdir);
            filesProcessed += process_sub(data_file, struct_file);
            CHDIR("..");
            curSubdir[sublen] = 0;
          } else {
            LWIP_LOGE("WARNING: cannot process sub due to path length restrictions: \"%s/%s\"\n", curSubdir, currName);
          }
        }
      }
    }
        ...
        if (ret == 0) {
          if (!file.is_dir) {
            ...
            if (file_to_exclude(curName)) {
              LWIP_LOGE("skipping %s/%s by exclude list (-x option)..." NEWLINE, curSubdir, curName);
              continue;
            }

            LWIP_LOGD("processing %s/%s..." NEWLINE, curSubdir, curName);

            if (process_file(data_file, struct_file, curName) < 0) {
              LWIP_LOGE(NEWLINE "Error... aborting" NEWLINE);
              return -1;
            }
            filesProcessed++;
          }
    ...
    if (can_be_compressed) {
      if (fsize < OUT_BUF_SIZE) {
        ...
        if (out_bytes < fsize) {
          ...
          buf = ret_buf;
          *file_size = out_bytes;
          LWIP_LOGI(" - deflate: %d bytes -> %d bytes (%.02f%%)" NEWLINE, (int)fsize, (int)out_bytes, (float)((out_bytes * 100.0) / fsize));
          deflatedBytesReduced += (size_t)(fsize - out_bytes);
          *is_compressed = 1;
        } else {
          LWIP_LOGE(" - uncompressed: (would be %d bytes larger using deflate)" NEWLINE, (int)(out_bytes - fsize));
        }
      } else {
        LWIP_LOGE(" - uncompressed: (file is larger than deflate bufer)" NEWLINE);
      }
    } else {
      LWIP_LOGE(" - cannot be compressed" NEWLINE);
    }
  }
#else
...
static void fix_filename_for_c(char *qualifiedName, size_t max_len)
{
  ...
  size_t i;
  if (len + 3 == max_len) {
    LWIP_LOGE("File name too long: \"%s\"\n", qualifiedName);
    exit(-1);
  }
  ...
  if (!filename_ok) {
    LWIP_LOGE("Failed to get unique file name: \"%s\"\n", qualifiedName);
    exit(-1);
  }
...
static int checkSsiByFilelist(const char* filename_listfile)
{
  ...
  if (f != NULL) {
    ...
    if (!buf) {
      LWIP_LOGE("failed to allocate ssi file buffer\n");
      fclose(f);
      return 0;
    }
    memset(buf, 0, fsize);
    ...
    if ((readcount > fsize) || !readcount) {
      LWIP_LOGE("failed to read data from ssi file\n");
      free(buf);
      return 0;
    }
    ...
    if (!lines) {
      LWIP_LOGE("failed to allocate ssi line buffer\n");
      free(buf);
      return 0;
    }
                           ...
                           u16_t *http_hdr_chksum, u8_t provide_content_len, int is_compressed)
{
  ...
  if ((file_ext == NULL) || (*file_ext == 0)) {
    LWIP_LOGE("failed to get extension for file \"%s\", using default.\n", filename);
    ...
    if (file_type == NULL) {
      LWIP_LOGE("failed to get file type for extension \"%s\", using default.\n", file_ext);
      file_type = HTTP_HDR_DEFAULT_TYPE;
    }
  }
  ...
  if (provide_last_modified) {
    ...
    if (stat(filename, &stat_data) != 0) {
      LWIP_LOGE("stat(%s) failed with error %d\n", filename, errno);
      exit(-1);
    }
    t = gmtime(&stat_data.st_mtime);
    if (t == NULL) {
      LWIP_LOGE("gmtime() failed with error %d\n", errno);
      exit(-1);
    }
