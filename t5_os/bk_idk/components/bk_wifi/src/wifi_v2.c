...
bk_err_t bk_wifi_init(const wifi_init_config_t *config)
{
	...
	if (bk_pm_low_voltage_register_wrapper(wifi_lowvol_exit_cb))
		WIFI_LOGW("register wifi pm low voltage cb fail!\n");
...
bk_err_t bk_wifi_scan_get_result(wifi_scan_result_t *scan_result)
{
	...
	scan_result->ap_num = ap_list.ApNum;
	scan_result->aps = os_zalloc(sizeof(wifi_scan_ap_info_t) * scan_result->ap_num);
	if (!scan_result->aps) {
		ret = BK_ERR_NO_MEM;
		WIFI_LOGW("ap_num=%u oom\n", ap_list.ApNum);
		goto _free_and_exit;
	}

	for (int i = 0; i < scan_result->ap_num; i++) {
		ap = &scan_result->aps[i];
...
static bk_err_t wifi_ap_set_config(const wifi_ap_config_t *ap_config)
{
	...
	if(ap_config->max_con == 0 || ap_config->max_con > CFG_SUPPORTED_MAX_STA_NUM) {
		if(ap_config->max_con == 0)
			WIFI_LOGD("max_con is zero, set it to default\n");
		if(ap_config->max_con > CFG_SUPPORTED_MAX_STA_NUM)
			WIFI_LOGD("invalid max_con, set it to default\n");
