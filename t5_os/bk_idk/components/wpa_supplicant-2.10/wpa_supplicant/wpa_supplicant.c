				   ...
				   u16 reason_code)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "Request to deauthenticate - bssid=" MACSTR
		...
		wpa_supplicant_state_txt(wpa_s->wpa_state));
#elif defined(BK_SUPPLICANT)
	WPA_LOGI("Request to deauthenticate, bssid %pm, "
		"pending_bssid %pm, reason %d, state %d, caller %p\n",
		wpa_s->bssid, wpa_s->pending_bssid,
		reason_code, wpa_s->wpa_state, __builtin_return_address(0));
#endif
