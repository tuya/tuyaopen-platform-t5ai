                        ...
                        beken_thread_function_t function, uint32_t stack_size, beken_thread_arg_t arg )
{
    ...
    if( pdPASS == xTaskCreateInPsram( (native_thread_t)function, name, 
            (unsigned short) (stack_size/sizeof( portSTACK_TYPE )), 
            (void *)arg, BK_PRIORITY_TO_NATIVE_PRIORITY(priority), 
            (TaskHandle_t *)thread ) )
    {
...
bk_err_t rtos_trylock_recursive_mutex(beken_mutex_t *mutex)
{
    if ( xSemaphoreTakeRecursive( *mutex, 0 ) != pdPASS )
    {
        return kGeneralErr;
    }
...
#if CONFIG_SOFTWARE_DECODE_SRAM_MAPPING || CONFIG_SIZE_OPTIMIZE
