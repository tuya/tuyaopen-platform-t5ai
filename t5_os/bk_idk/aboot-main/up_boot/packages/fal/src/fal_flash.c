...
int fal_flash_init(void)
{
        ...
        for (i = 0; i < device_table_len; i++)
        {
                ...
                log_d("Flash | %*.*s | addr: 0x%08lx | len: 0x%08x | blk_size: 0x%08x |init finish.",
 ...
 */
int fal_get_fw_hdr(const char *part_name, struct ota_rbl_hdr *hdr)
{
	const struct fal_partition *part = fal_partition_find(part_name);
	
	if (!strcmp(part_name, RT_BK_DL_PART_NAME))
    {
    	/* firmware header is on OTA download partition top */
        fal_partition_read(part, 0, (uint8_t *) hdr, sizeof(struct ota_rbl_hdr));
    }
	else
	{
