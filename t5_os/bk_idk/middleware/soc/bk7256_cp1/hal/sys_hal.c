...
void sys_hal_enter_deep_sleep(void * param)
{
	...
	sys_ll_set_ana_reg6_en_dpll(0);

	/*4.set PMU parameters*/
	//pmu_inst->REG0X40 = 0x4e111111;
...
void sys_hal_enter_low_voltage(void)
{
	...
	/*3.set sleep parameters*/
	//pmu_inst->REG0X40 = 0x6B111111;
	aon_pmu_hal_set_sleep_parameters(0x6B111111);
...
void sys_hal_wifiorbt_wakeup_enable(uint32_t type)
{
	...
	if(type == 0){
		sys_ll_set_cpu_power_sleep_wakeup_wifi_wakeup_platform_en(0);
	}else{
		sys_ll_set_cpu_power_sleep_wakeup_bts_wakeup_platform_en(0);
	}
...
/*for bringup temp test function start*/
...
void sys_hal_module_RF_power_ctrl (module_name_t module,power_module_state_t power_state)
{
    ...
    if(power_state == POWER_MODULE_STATE_ON)//power on
    {
		value |= (1 << 13);//en_sysldo
		value |= (1 << 12);//en_dpll  //fisrt test not open dpll,using the default xtl26m
		//value |= (1 << 11);//en_audpll //fisrt test not open en_audpll,using the default xtl26m
		value |= (1 << 10);//tempreture det enable
		value |= (1 << 8);//en_dco
		value |= (1 << 7);//en_xtall
    }
	else //power down
...
uint32 sys_hal_get_intr_raw_status(void)
{
	...
	///TODO:
	reg = sys_ll_get_cpu1_int_0_31_status_value();
...
void sys_hal_btdm_interrupt_ctrl(bool en)
{
    if (en)
    {
        sys_ll_set_cpu1_int_32_63_en_cpu1_dm_irq_en(1);
            //TODO enable PLIC Int Enable Registers
    }
    else
    {
        sys_ll_set_cpu1_int_32_63_en_cpu1_dm_irq_en(0);
...
void sys_hal_ble_interrupt_ctrl(bool en)
{
    if (en)
    {
        sys_ll_set_cpu1_int_32_63_en_cpu1_ble_irq_en(1);
            //TODO enable PLIC Int Enable Registers
    }
    else
    {
        sys_ll_set_cpu1_int_32_63_en_cpu1_ble_irq_en(0);
            //TODO disable PLIC Int Enable Registers
    }
}
...
void sys_hal_bt_interrupt_ctrl(bool en)
{
    if (en)
    {
        sys_ll_set_cpu1_int_32_63_en_cpu1_bt_irq_en(1);
            //TODO enable PLIC Int Enable Registers
    }
    else
    {
        sys_ll_set_cpu1_int_32_63_en_cpu1_bt_irq_en(0);
            //TODO disable PLIC Int Enable Registers
    }
}
...
{
	sys_ll_set_cpu1_int_32_63_en_cpu1_wifi_mac_int_gen_en(1);
}
void sys_hal_enable_mac_prot_int(void)
{
	sys_ll_set_cpu1_int_32_63_en_cpu1_wifi_mac_int_prot_trigger_en(1);
}
void sys_hal_enable_mac_tx_trigger_int(void)
{
	sys_ll_set_cpu1_int_32_63_en_cpu1_wifi_mac_int_tx_trigger_en(1);
}
void sys_hal_enable_mac_rx_trigger_int(void)
{
	sys_ll_set_cpu1_int_32_63_en_cpu1_wifi_mac_int_rx_trigger_en(1);
}
void sys_hal_enable_mac_txrx_misc_int(void)
{
	sys_ll_set_cpu1_int_32_63_en_cpu1_wifi_mac_int_tx_rx_misc_en(1);
}
void sys_hal_enable_mac_txrx_timer_int(void)
{
	sys_ll_set_cpu1_int_0_31_en_cpu1_wifi_mac_int_tx_rx_timer_en(1);
}

void sys_hal_enable_modem_int(void)
{
	sys_ll_set_cpu1_int_0_31_en_cpu1_wifi_int_phy_mpb_en(1);
}
void sys_hal_enable_modem_rc_int(void)
{
	sys_ll_set_cpu1_int_0_31_en_cpu1_wifi_int_phy_riu_en(1);
}
...
{
	sys_ll_set_ana_reg16_diffen(value);
}
...
{
	sys_ll_set_cpu1_int_0_31_en_cpu1_aud_int_en(value);
}

void sys_hal_sbc_int_en(uint32_t value)
{
	sys_ll_set_cpu1_int_0_31_en_cpu1_sbc_int_en(value);
}
...
{
	sys_ll_set_cpu1_int_0_31_en_cpu1_fft_int_en(value);
}
...
{
	sys_ll_set_cpu1_int_0_31_en_cpu1_i2s_int_en(value);
}
...
{
	return (sys_ll_get_ana_reg6_value() & (0x1 << 9));
}
...
void sys_hal_psram_psldo_vset(uint32_t output_voltage, uint32_t is_add_200mv)
{
	...
	sys_ll_set_ana_reg9_psldo_vsel(is_add_200mv); // 1:add 200mv, 0:not add
}
