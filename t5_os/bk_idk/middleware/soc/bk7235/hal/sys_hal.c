...
void sys_hal_buck_switch(uint32_t enable)
{
	if(enable)
	{
		sys_ll_set_ana_reg2_value(0x6367F09E);//bit26: ldo/buck select, 0:buck;1:LDO
		sys_ll_set_ana_reg3_value(0x92188808);
	}
	else
	{   /*restore default value*/
		sys_ll_set_ana_reg2_value(0x676EF27E);
		sys_ll_set_ana_reg3_value(0xE39881CE);
	}
}
...
bk_err_t sys_hal_core_bus_clock_ctrl(uint32_t cksel_core, uint32_t ckdiv_core,uint32_t ckdiv_bus, uint32_t ckdiv_cpu0,uint32_t ckdiv_cpu1)
{
	...
	if((ckdiv_core > PM_FREQUNCY_DIV_MAX) || (ckdiv_bus > PM_FREQUNCY_DIV_BUS_MAX)||(ckdiv_cpu0 > PM_FREQUNCY_DIV_MAX)||(ckdiv_cpu1 > PM_FREQUNCY_DIV_MAX))
	{
...
/*adjust the vdddig voltage*/
static bk_err_t sys_hal_adjust_vdddig_voltage(pm_cpu_freq_e cpu_freq)
{
	uint32_t reg_value;
	uint32_t param = 0;
	if(cpu_freq == PM_CPU_FRQ_480M)
	{
		param = PM_VDDDIG_1P3V;
	}
	else
	{
		param = PM_VDDDIG_1P15V;
	}

	reg_value = sys_ll_get_ana_reg3_vhsel_ldodig();

	if(reg_value == param)
	{
	    return BK_OK;//have setted, it not need set again.
	}
	...
	delay_us(SWITCH_CPU_FREQ_DELAY);
	return BK_OK;
}
bk_err_t sys_hal_switch_cpu_bus_freq(pm_cpu_freq_e cpu_bus_freq)
{
    bk_err_t ret = BK_OK;
	sys_hal_adjust_vdddig_voltage(cpu_bus_freq);
    switch(cpu_bus_freq)
    {
    	case PM_CPU_FRQ_480M://cpu0:480m;cpu1:480m;bus:240m
			ret = sys_hal_core_bus_clock_ctrl(0x3,0x0,0x1,0x0,0x0);
			break;
		case PM_CPU_FRQ_320M://cpu0:320m;cpu1:320m;bus:160m
			ret = sys_hal_core_bus_clock_ctrl(0x2,0x0,0x1,0x0,0x0);
			break;
		case PM_CPU_FRQ_240M://cpu0:240m;cpu1:240m;bus:120m
			ret = sys_hal_core_bus_clock_ctrl(0x3,0x1,0x1,0x0,0x0);
			break;
...
{
    /*TODO set bk7235 saradc ananlog reg config*/
}
...
//	sys_ll_set_cpu0_int_0_31_en_cpu0_dma2d_int_en(sys_int_en); //check
}
...
void sys_hal_cpu0_fft_int_en(uint32_t value)
{
...
{
    sys_ll_set_ana_reg8_en_adcmode(value);
}
