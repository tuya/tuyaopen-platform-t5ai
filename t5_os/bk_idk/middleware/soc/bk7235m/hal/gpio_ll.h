...
static inline void gpio_ll_set_interrupt_type(gpio_hw_t *hw, uint32 index, gpio_int_type_t mode)
{
	if (index < GPIO_16)
		...
		REG_MCHAN_SET_FIELD(index-GPIO_32, &hw->gpio_32_47_int_type, GPIO_F_INT_TYPE_MODE, mode);
...
static inline void gpio_ll_clear_interrupt_status(gpio_hw_t *hw, gpio_interrupt_status_t *gpio_status)
{
	if (gpio_status->gpio_0_31_int_status)
		REG_WRITE(&hw->gpio_0_31_int_clr, gpio_status->gpio_0_31_int_status);
	if (gpio_status->gpio_32_64_int_status)
		REG_WRITE(&hw->gpio_32_47_int_clr, gpio_status->gpio_32_64_int_status);
...
#if 0
	if (index < GPIO_32)
		REG_MCHAN_SET_FIELD(index, &hw->gpio_0_31_int_clr, GPIO_F_INT_EN, 1);
	else
		REG_MCHAN_SET_FIELD(index-GPIO_32, &hw->gpio_32_47_int_clr, GPIO_F_INT_EN, 1);
#endif
	if (index < GPIO_32)
		hw->gpio_0_31_int_clr.v = (1<<index);
	else if (index < GPIO_NUM)
		hw->gpio_32_47_int_clr.v = (1<<(index - 32));
}
