...
/** Platform Misc End **/
void sys_hal_not_supported(void)
{
	while(8778);
}
...
/*
	...
	//For BK7286 ckdiv_bus is the same as ckdiv_cpu1 0x5[4]
...
bk_err_t sys_hal_switch_cpu_bus_freq(pm_cpu_freq_e cpu_bus_freq)
{
    ...
    switch(cpu_bus_freq)
    {
		case PM_CPU_FRQ_320M://cpu0:160m;cpu1:320m;bus:160m
			...
			sys_hal_ctrl_vdddig_h_vol(0x8);//0.8V     //When the voting frequency is greater than PM_CPU_FRQ_240M, set PM_CPU_FRQ_240M
...
{
	/*TODO check bk7296 saradc ananlog reg config*/
}

void sys_hal_set_sdmadc_config(void)
{
	/*TODO check bk7286 sdmadc ananlog reg config*/
...
{
	sys_ll_set_cpu_26m_wdt_clk_div_clksel_auxs_cis(value);
}
...
{
	sys_ll_set_ana_reg5_swb(enable); //1-1.8v, 0-3.0v
}
...
{
	sys_ll_set_ana_reg5_en_ram(enable);
}
...
{
	sys_ll_set_ana_reg5_vsel(value);// 0:add 200mv, 1:not add
}

void sys_hal_psram_psldo_vset(uint32_t output_voltage, uint32_t is_add_200mv)
{
	sys_ll_set_ana_reg5_swb(output_voltage); //1-1.8v, 0-3.0v
	sys_ll_set_ana_reg5_vsel(is_add_200mv);// 0:add 200mv, 1:not add
}
...
void sys_hal_early_init(void)
{
	...
	sys_hal_analog_set(ANALOG_REG0, 0xF1325B57);  // triger dpll
	 ...
	 */
	sys_ll_set_ana_reg9_spi_latch1v(1);
	if ((chip_id & PM_CHIP_ID_MASK) == (PM_CHIP_ID_MPW_V2 & PM_CHIP_ID_MASK)) {
	...
	/* pujie20230720: improve GPIO drive capability */
	aon_pmu_ll_set_r41_flshsck_iocap(2);
