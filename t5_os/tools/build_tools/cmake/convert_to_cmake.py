...
def get_make_variables(path, makefile="Makefile", expected_failure=False, variables={}):
    ...
    Overrides ARMINO_PATH= to avoid recursively evaluating the entire project Makefile structure.
    """
...
def get_component_variables(project_path, component_path):
    make_vars = get_make_variables(component_path,
                                   os.path.join(os.environ["ARMINO_PATH"],
                                                "make",
...
""")
        f.write("""
include($ENV{ARMINO_TOOLS_PATH}/cmake/project.cmake)
""")
        f.write("project(%s)\n" % project_name)

    print("ARMINO_TOOLS_PATH: %s" % $ENV{ARMINO_TOOLS_PATH})
...
def convert_component(project_path, component_path):
    ...
    with open(cmakelists_path, "w") as f:
        if component_srcs is not None:
            f.write("armino_component_register(SRCS %s)\n" % component_srcs)
            ...
            f.write("armino_component_register()\n")
