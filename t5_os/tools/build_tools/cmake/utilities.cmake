function(LOGI str)
	message(STATUS "${Green}${str}${ColourReset}")
endfunction()

function(LOGW str)
	message(WARNING "${BoldBlue}${str}${ColourReset}")
endfunction()

function(LOGE str)
	message(SEND_ERROR "${BoldRed}${str}${ColourReset}")
endfunction()

function(LOGV str)
	message(VERBOSE "${BoldRed}${str}${ColourReset}")
endfunction()
...
function(move_if_different source destination)
    ...
    if(do_copy)
        LOGI("Moving ${source} -> ${destination}")
        file(RENAME ${source} ${destination})
    else()
        LOGI("Not moving ${source} -> ${destination}")
        file(REMOVE ${source})
    endif()
...
function(target_add_binary_data target embed_file embed_type)
    ...
    add_custom_command(OUTPUT "${embed_srcfile}"
        ...
        -P "${armino_tools_path}/cmake/scripts/data_file_embed_asm.cmake"
        MAIN_DEPENDENCY "${embed_file}"
        DEPENDS "${armino_tools_path}/cmake/scripts/data_file_embed_asm.cmake" ${__DEPENDS}
...
function(target_linker_script_judge)
	...
	set(nds_files_path ${armino_tools_path}/build_tools/nds_files)
	set(nds_ldsag_cmd ${nds_files_path}/nds_ldsag)
	set(sag_file)
	set(ld_file)
	set(ldsag_flag 1)
	set(link_flag 1)
	#LOGI("PROJECT_DIR is $ENV{PROJECT_DIR}")
	if (EXISTS $ENV{PROJECT_DIR}/${target}/${target}_bsp.sag)
		set(sag_file $ENV{PROJECT_DIR}/${target}/${target}_bsp.sag)
		set(ld_file $ENV{PROJECT_DIR}/${target}/${target}_bsp.ld)
	elseif(EXISTS $ENV{PROJECT_DIR}/${target}_bsp.sag)
		set(sag_file $ENV{PROJECT_DIR}/${target}_bsp.sag)
		set(ld_file $ENV{PROJECT_DIR}/${target}_bsp.ld)
	elseif(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${target}_bsp.sag)
		set(sag_file ${CMAKE_CURRENT_SOURCE_DIR}/${target}_bsp.sag)
		set(ld_file ${CMAKE_CURRENT_SOURCE_DIR}/${target}_bsp.ld)
	else()
		#nothing todo
		set(ldsag_flag 0)
	endif()
	if(${ldsag_flag} STREQUAL 1)
		execute_process(COMMAND
			${nds_ldsag_cmd} --version=v5 -t ${nds_files_path}/nds32_template_v5_linux.txt ${sag_file} -o ${ld_file}
		...
		if(result STREQUAL 0)
			LOGI("nds_ldsag_cmd execute success...")
		else()
			set(link_flag 0)
			LOGI("nds_ldsag_cmd execute failed!!!")
		endif()
	else()
        if($ENV{PROJECT} STREQUAL "cpu1")
            set(ld_file ${CMAKE_CURRENT_SOURCE_DIR}/${target}_cpu1.ld)
		elseif(EXISTS $ENV{PROJECT_DIR}/${target}/${target}_bsp.ld)
			set(ld_file $ENV{PROJECT_DIR}/${target}/${target}_bsp.ld)
		elseif(EXISTS $ENV{PROJECT_DIR}/${target}_bsp.ld)
			set(ld_file $ENV{PROJECT_DIR}/${target}_bsp.ld)
		elseif(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${target}_bsp.ld)
			set(ld_file ${CMAKE_CURRENT_SOURCE_DIR}/${target}_bsp.ld)
		else()
			set(ld_file)
			set(link_flag 0)
	...
	if(${link_flag} STREQUAL 1)
		target_linker_script(${COMPONENT_LIB} INTERFACE "${ld_file}")
	else()
		LOGI("sag_file is ${sag_file}")
		LOGI("ld_file is ${ld_file}")
		LOGI("ldsag_flag is ${ldsag_flag}")
		LOGI("link_flag is ${link_flag}")
		LOGE("linker script not exist!!!")
	endif()
endfunction()
...
function(fail_at_build_time target_name message_line0)
    ...
    add_custom_target(${target_name} ALL
        ...
        COMMAND ${CMAKE_COMMAND} -E env FAIL_MESSAGE=${fail_message}
                ${CMAKE_COMMAND} -P ${armino_path}/tools/cmake/scripts/fail.cmake
        VERBATIM)
endfunction()
...
function(check_exclusive_args args prefix)
    ...
    foreach(arg ${_args})
        if(${prefix}_${arg} AND only_arg)
            LOGE("${args} are exclusive arguments")
        endif()
...
function(add_prebuilt_library target_name lib_path)
    ...
    foreach(req ${__REQUIRES})
        armino_component_get_property(req_lib "${req}" COMPONENT_LIB)
...
function(file_generate output)
    ...
    elseif(__CONTENT)
        ...
        LOGE("Content to generate not specified.")
    endif()
