...
function(__build_set_default_build_specifications)
    ...
    if (EXISTS ${soc_path}/${armino_target}/compile-options.cmake)
        ...
        if (DEFINED OVERRIDE_LINK_OPTIONS)
            list(APPEND link_options ${OVERRIDE_LINK_OPTIONS})
        endif()
...
function(find_cmake_lists_dirs root_dir)
    set(max_depth 8)
    set(current_depth 0)
    set(dirs_to_search ${root_dir})

    set(result_dirs "")
    while(dirs_to_search AND current_depth LESS max_depth)
        set(new_dirs_to_search "")
        foreach(dir ${dirs_to_search})
            ...
            foreach(sub_dir ${sub_dirs})
                if(IS_DIRECTORY "${dir}/${sub_dir}")
                    list(APPEND new_dirs_to_search "${dir}/${sub_dir}")
                endif()
            endforeach()
        endforeach()
        set(dirs_to_search ${new_dirs_to_search})
        math(EXPR current_depth "${current_depth} + 1")
    endwhile()

    set(sub_dirs_list ${result_dirs} PARENT_SCOPE)
endfunction()
...
function(__build_add_components components_path)
    ...
    foreach(property_dir ${property_dirs})
        ...
        if(is_loop_sub_dirs)
            find_cmake_lists_dirs(${property_dir})
...
function(__build_init armino_path)
    ...
    armino_build_set_property(ARMINO_TOOLS_PATH "$ENV{ARMINO_TOOLS_PATH}")  
    LOGI("ARMINO_TOOLS_PATH is $ENV{ARMINO_TOOLS_PATH}")
    ...
    set(requires_common include bk_common common part_table)
...
function(__build_check_python)
    ...
    if(check)
        ...
        LOGI("ARMINO_TOOLS_PATH is ${armino_tools_path}")

        LOGI("Checking Python dependencies...")
        execute_process(COMMAND "${python}" "${armino_tools_path}/build_tools/check_python_dependencies.py"
            RESULT_VARIABLE result)
...
function(armino_build_executable bin)
    ...
    add_custom_target(size-statistic ALL DEPENDS gen_project_binary
        ...
        COMMAND ${armino_size_parse_map} "${build_dir}/app.map"
    )
...
function(armino_build_parse_config_file_list)
    ...
    if(NOT _sdkconfig_defaults)
        ...
        if(EXISTS "${CMAKE_SOURCE_DIR}/${sub_sys}/config/common/config")
            list(APPEND _sdkconfig_defaults "${CMAKE_SOURCE_DIR}/${sub_sys}/config/common/config")
        elseif(EXISTS "${CMAKE_SOURCE_DIR}/${sub_sys}/config/common.config")
            list(APPEND _sdkconfig_defaults "${CMAKE_SOURCE_DIR}/${sub_sys}/config/common.config")
        endif()

        if(EXISTS "${CMAKE_SOURCE_DIR}/${sub_sys}/config/${ARMINO_SOC}/config")
            list(APPEND _sdkconfig_defaults "${CMAKE_SOURCE_DIR}/${sub_sys}/config/${ARMINO_SOC}/config")
        elseif(EXISTS "${CMAKE_SOURCE_DIR}/${sub_sys}/config/${ARMINO_SOC}.config")
            list(APPEND _sdkconfig_defaults "${CMAKE_SOURCE_DIR}/${sub_sys}/config/${ARMINO_SOC}.config")
        endif()
        ...
        if(EXISTS "${soc_path}/${ARMINO_SOC}/${ARMINO_SOC}.defconfig")
            if(build_bklibs EQUAL 0 AND EXISTS "$ENV{MODULE_DIR}/${ARMINO_SOC}/$ENV{MODULE}.defconfig")
                set(_sdkconfig_default_soc "$ENV{MODULE_DIR}/${ARMINO_SOC}/$ENV{MODULE}.defconfig")
            else()
...
function(armino_build_parse_toolchain_dir)
    ...
    string(STRIP "$ENV{COMPILER_TOOLCHAIN_PATH}" toolchain_dir)
    # set(toolchain_dir $ENV{COMPILER_TOOLCHAIN_PATH})

    # if(IS_ABSOLUTE ${toolchain_dir})
    # else()
    #     LOGE("please set compiler toolchain as absoulate path")
    # endif()
