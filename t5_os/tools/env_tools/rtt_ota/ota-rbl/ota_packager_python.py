...
PARTITIONS_DIR = os.environ['PARTITIONS_DIR']
...
def packager(args):
    ...
    if os.path.exists('%s/partitions/%s/ota_rbl.config'%(project_dir,armino_soc)) and os.path.exists('%s/configurationab.json'%(PARTITIONS_DIR)):
        b_new_chip = 1
        config = '%s/partitions/%s/ota_rbl.config'%(project_dir,armino_soc)
        print('=========================>>>> b_new_chip = 1')
    #print("============>>>>",config)
    ...
    with open(args.outfile,"wb") as f:
        if b_new_chip == 1:
            ...
            config = '%s/configurationab.json'%(PARTITIONS_DIR)
...
def parse_args():
    ...
    parser.add_argument('-soc', '--soc_dir', default=None, help="sdk armino_soc dir")
