...
//static TCell    *buffer = NULL;
  ...
  gray_convert_glyph( RAS_ARG )
  {
    ...
    {
        bk_printf("[%s][%d] enter\r\n", __FUNCTION__, __LINE__);
        buffer = (TCell *)os_malloc(sizeof(TCell)*FT_MAX_GRAY_POOL);
        if(!buffer)
        {
            os_printf("[gray_convert_glyph] malloc fail\r\n");
            return -1;
        }
        else
        {
            os_printf("[gray_convert_glyph] malloc success:%d\r\n", sizeof(TCell)*FT_MAX_GRAY_POOL);
        }
    }
#endif
