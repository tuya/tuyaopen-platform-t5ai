...
{
    return indev->proc.types.pointer.start_point.x;
}

lv_coord_t lv_indev_get_start_point_y(const lv_indev_t * indev)
{
    return indev->proc.types.pointer.start_point.y;
}
// Modified by TUYA End
...
// Modified by TUYA End
    if(i->proc.state == LV_INDEV_STATE_PRESSED) {
        ...
        // Modified by TUYA Start
        lv_obj_t * scroll_obj = i->proc.types.pointer.scroll_obj;
...
// Modified by TUYA Start
        lv_event_send(lv_scr_act(), LV_EVENT_GESTURE_RELEASED, NULL);
        if(indev_reset_check(proc)) return;
// Modified by TUYA End
        lv_event_send(indev_obj_act, LV_EVENT_RELEASED, indev_act);
        if(indev_reset_check(proc)) return;
...
void indev_gesture(_lv_indev_proc_t * proc)
{
    ...
    if((LV_ABS(proc->types.pointer.vect.x) < indev_act->driver->gesture_min_velocity) &&
       (LV_ABS(proc->types.pointer.vect.y) < indev_act->driver->gesture_min_velocity)) {
        ...
        proc->types.pointer.gesture_sum.y = 0;
        // Modified by TUYA Start
        lv_memset(&proc->types.pointer.start_point, 0x0, sizeof(lv_point_t));
        // Modified by TUYA End
    }
    ...
    // Modified by TUYA Start
    if (!proc->types.pointer.start_point.x && !proc->types.pointer.start_point.y) {
        lv_memcpy(&proc->types.pointer.start_point, &proc->types.pointer.act_point, sizeof(lv_point_t));
    }
    // Modified by TUYA End
