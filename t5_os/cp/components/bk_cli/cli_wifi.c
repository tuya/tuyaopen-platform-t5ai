...
#if CONFIG_WIFI_VNET_CONTROLLER
beken_timer_t cif_dbg_timer;

void cif_debug_timer()
{
	...
	CLI_LOGE("tx avail %d, rx avail %d, tx used %d, rx used %d, tx max %d, rx max %d, tx err %d, rx err %d\r\n",
			MEM_MAX_TX_SIZE,
			MEM_MAX_RX_SIZE,
	...
	rwm_get_mac_trx_status(0);
	count++;
}

void cif_debug_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	//int ret = 0;
	//char *msg = NULL;

	if (argc < 2) {
		CIF_LOGE("invalid cif command %d\n");
		CIF_LOGE("Only support following command\n");
		CIF_LOGE("CIF wakeup_host\n");
		CIF_LOGE("CIF proto_dbg_en 0/1\n");
		CIF_LOGE("CIF stats\n");
		return;
	}

	if (os_strcmp(argv[1], "proto_dbg_en") == 0) {
//		#if CONFIG_RWNX_PROTO_DEBUG
//		uint8_t dbg_en = (uint8_t)os_strtoul(argv[2], NULL, 10);
//		CLI_LOGE("set proto debug flag:%d\n", dbg_en);
//		rwm_set_proto_debug(dbg_en);
//		#endif
	}
		...
		uint16_t time = 0;
		int ret = 0;
		if (argc < 3) CIF_LOGE("cif stats_auto 1000\n");
		time = (uint16_t)os_strtoul(argv[2], NULL, 10);

		if(time != 0)
		{
			/* Create a new software timer,software is AutoReload */
			ret = rtos_init_timer(&cif_dbg_timer, time, cif_debug_timer, 0);
			if (ret != BK_OK)
			{
				CIF_LOGE("debug_timer init failed:%d\n", ret);
			}
			ret = rtos_start_timer(&cif_dbg_timer);
			if (ret != BK_OK)
			{
				CIF_LOGE("debug_timer start failed:%d\n", ret);
			}

		}
		else
		{
			ret = rtos_stop_timer(&cif_dbg_timer);
			if (ret != BK_OK)
			{
				CIF_LOGE("debug_timer stop failed:%d\n", ret);
			}
		}
	}
	else
	{
		CIF_LOGE("invalid paramter\n");
	}
}
#endif
...
#endif
	#if CONFIG_WIFI_VNET_CONTROLLER
	{"cif", "controller test cmd", cif_debug_cmd},
	#endif
};
