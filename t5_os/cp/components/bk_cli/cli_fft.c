...
{
	BK_LOGD(NULL,"fft_fft_test {start|stop}\r\n");
	BK_LOGD(NULL,"fft_ifft_test {start|stop}\r\n");
	BK_LOGD(NULL,"fft_fir_signal_test {start|stop} \r\n");
	BK_LOGD(NULL,"fft_fir_dual_test {start|stop} \r\n");
}
...
void cli_fft_fft_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "start") == 0) {
		BK_LOGD(NULL,"fft test start\n");

		//init fft driver
		ret = bk_fft_driver_init();
		if (ret != BK_OK)
			return;
		BK_LOGD(NULL,"fft driver successful\r\n");
		...
		for (i = 0; i < FFT_TEST_DATA_SIZE; i++) {
			...
			//BK_LOGD(NULL,"0x%08x, ", fft_conf.inbuf[i]);
		}
		BK_LOGD(NULL,"\r\n");
		//start fft
		bk_fft_enable(&fft_conf);
		BK_LOGD(NULL,"start fft process \r\n");
		//fft_struct_dump();

		//wait fft complete
		while(bk_fft_is_busy())
			;
		BK_LOGD(NULL,"fft complete\r\n");
		...
		BK_LOGD(NULL,"\r\ndata_proc:\n");
		for (i = 0; i < FFT_TEST_DATA_SIZE; i++) {
			//BK_LOGD(NULL,"0x%04hx%04hx, ", data_proc_q[i], data_proc_i[i]);
		...
		if (ret == BK_OK)
			BK_LOGD(NULL,"start fft test successful\r\n");
		else
			BK_LOGD(NULL,"start fft test failed\r\n");

	} else if (os_strcmp(argv[1], "stop") == 0) {
		BK_LOGD(NULL,"fft test stop\n");
		bk_fft_driver_deinit();
		BK_LOGD(NULL,"fft test stop successful\n");
	} else {
		cli_fft_help();
		return;
	}
...
void cli_fft_ifft_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "start") == 0) {
		BK_LOGD(NULL,"ifft test start\n");

		//init fft driver
		ret = bk_fft_driver_init();
		if (ret != BK_OK)
			return;
		BK_LOGD(NULL,"ifft driver successful\r\n");
		...
		//start fft
		bk_fft_enable(&fft_conf);
		BK_LOGD(NULL,"start ifft process \r\n");

		//wait fft complete
		while(bk_fft_is_busy())
			;
		BK_LOGD(NULL,"ifft complete\r\n");
		...
		if (ret == BK_OK)
			BK_LOGD(NULL,"start ifft test successful\r\n");
		else
			BK_LOGD(NULL,"start ifft test failed\r\n");

	} else if (os_strcmp(argv[1], "stop") == 0) {
		BK_LOGD(NULL,"ifft test stop\n");
		bk_fft_driver_deinit();
		BK_LOGD(NULL,"ifft test stop successful\n");
	} else {
		cli_fft_help();
		return;
	}
