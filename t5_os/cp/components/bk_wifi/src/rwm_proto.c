...
static void rwm_udp_printf(char *dir, struct ipv4_hdr *ip_hdr)
{
	...
	if (((htons(DHCP_SPORT(udphdr)) == 68) && (htons(DHCP_DPORT(udphdr)) == 67)) ||
		...
		if ((dmsg->options[2] & 0xff) == 0x01)
			RWNX_LOGD("%s DHCP-discover  id %0x(%u)\n", dir, ntohs(ip_hdr->id), ntohs(ip_hdr->id));
		else if ((dmsg->options[2] & 0xff) == 0x02)
			RWNX_LOGD("%s DHCP-offer  id %0x(%u)\n", dir, ntohs(ip_hdr->id), ntohs(ip_hdr->id));
		else if ((dmsg->options[2] & 0xff) == 0x03)
			RWNX_LOGD("%s DHCP-request  id %0x(%u)\n", dir, ntohs(ip_hdr->id), ntohs(ip_hdr->id));
		else if ((dmsg->options[2] & 0xff) == 0x04)
			RWNX_LOGD("%s DHCP-decline  id %0x(%u)\n", dir, ntohs(ip_hdr->id), ntohs(ip_hdr->id));
		else if ((dmsg->options[2] & 0xff) == 0x05)
			RWNX_LOGD("%s DHCP-ACK  id %0x(%u)\n", dir, ntohs(ip_hdr->id), ntohs(ip_hdr->id));
		else if ((dmsg->options[2] & 0xff) == 0x06)
			RWNX_LOGD("%s DHCP-NAK  id %0x(%u)\n", dir, ntohs(ip_hdr->id), ntohs(ip_hdr->id));
		else if ((dmsg->options[2] & 0xff) == 0x07)
			RWNX_LOGD("%s DHCP-release  id %0x(%u)\n", dir, ntohs(ip_hdr->id), ntohs(ip_hdr->id));
		else if ((dmsg->options[2] & 0xff) == 0x08)
			RWNX_LOGD("%s DHCP-inform  id %0x(%u)\n", dir, ntohs(ip_hdr->id), ntohs(ip_hdr->id));
		else
			RWNX_LOGD("%s UDP, id %0x(%u)\n, Unknow DHCP type\n", dir, ntohs(ip_hdr->id), ntohs(ip_hdr->id));
	} else {
		/* other udp packets*/
		RWNX_LOGD("%s UDP, id %0x(%u)\n", dir, ntohs(ip_hdr->id), ntohs(ip_hdr->id));
	}
...
static void rwm_ip_printf(char *dir, struct ethhdr *eth_hdr_ptr)
{
	...
	case 1:     //IP_PROTO_ICMP
		RWNX_LOGD("%s ICMP, id %0x(%u)\n", dir, ntohs(iphdr->id), ntohs(iphdr->id));
		break;
	case 2:     //IP_PROTO_IGMP
		RWNX_LOGD("%s IGMP, id %0x(%u)\n", dir, ntohs(iphdr->id), ntohs(iphdr->id));
		break;
	case 17:     //IP_PROTO_UDP
		rwm_udp_printf(dir, iphdr);
		break;
	case 136:     //IP_PROTO_UDP_LITE
		RWNX_LOGD("%s UDP_LITE, id %0x(%u)\n", dir, ntohs(iphdr->id), ntohs(iphdr->id));
		break;
	case 6:     //IP_PROTO_TCP
		RWNX_LOGD("%s TCP, id %0x(%u)\n", dir, ntohs(iphdr->id), ntohs(iphdr->id));
		break;

	default:     //UNKNOW TYPE
		RWNX_LOGD("%s other ip type, id %0x(%u)\n", dir, ntohs(iphdr->id), ntohs(iphdr->id));
		break;
	}
}
...
static void rwm_arp_printf(char *dir, struct ethhdr *eth_hdr_ptr)
{
	...
	if (ntohs(ARP_OPCODE(arphdr)) == 1) {
		...
		RWNX_LOGD("%s ARP - Response\n", dir);
	} else {
		RWNX_LOGD("%s ERR ARP\n", dir);
		return;
	}
	RWNX_LOGD("Hardware: %u\n", ntohs(ARP_TYPE(arphdr)));
	RWNX_LOGD("Protocol: 0x%02x\n", ntohs(ARP_PROTO(arphdr)));
	RWNX_LOGD("Hardware Addr Length: 6\n");
	RWNX_LOGD("Protocol Addr Length: 4\n");
	RWNX_LOGD("Operation: %u\n", ntohs(ARP_OPCODE(arphdr)));
	RWNX_LOGD("Sender Hardware Addr: "BK_MAC_FORMAT "\n", BK_MAC_STR(arphdr->shwaddr));
	RWNX_LOGD("Sender Internet Addr: "BK_IP4_FORMAT " \r\n", BK_IP_STR(arphdr->sipaddr));
	RWNX_LOGD("Target Hardware Addr: "BK_MAC_FORMAT "\n", BK_MAC_STR(arphdr->dhwaddr));
	RWNX_LOGD("Target Internet Addr: "BK_IP4_FORMAT " \r\n", BK_IP_STR(arphdr->dipaddr));
}
...
static void rwm_ipv6_printf(char *dir, struct ethhdr *eth_hdr_ptr)
{
	...
	if (IP6_NEXT_DEADER(ip6hdr) == 0) {         //HOPBYHOP
		...
		RWNX_LOGD("%s DESTOPTS\n", dir);
	} else if (IP6_NEXT_DEADER(ip6hdr) == 136) {    //UDPLITE
		RWNX_LOGD("%s UDPLITE\n", dir);
	} else {
		RWNX_LOGD("%s OTHER IPv6 type\n", dir);
	}
}
...
{
	if (eapol_cnt == 0) {
		RWNX_LOGD("%s Eapol frame process 1\n", dir);
		eapol_cnt++;
	} else if (eapol_cnt == 1) {
		RWNX_LOGD("%s Eapol frame process 2\n", dir);
		eapol_cnt++;
	} else if (eapol_cnt == 2) {
		RWNX_LOGD("%s Eapol frame process 3\n", dir);
		eapol_cnt++;
	} else {
		RWNX_LOGD("%s Eapol frame process 4\n", dir);
	}
}
...
void rwm_proto_printf(int direction, struct ethhdr *eth_hdr_ptr)
{
	...
	default:
		RWNX_LOGD("%s other ethernet type\n", dir);
		break;
	}
}
