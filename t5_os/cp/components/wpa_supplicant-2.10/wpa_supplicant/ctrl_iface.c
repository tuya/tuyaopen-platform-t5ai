	...
	struct wpa_supplicant *wpa_s, ScanResult_adv *results)
{
	...
	/* filter specified SSID */
	if (wpa_s->ssids_from_scan_req && wpa_s->ssids_from_scan_req->ssid_len) {
		ssid = wpa_s->ssids_from_scan_req->ssid;
		ssid_len = wpa_s->ssids_from_scan_req->ssid_len;
	}
...
int wpa_supplicant_ctrl_iface_set_network(struct wpa_supplicant *wpa_s, wlan_sta_config_t *config)
{
	...
	case WLAN_STA_FIELD_KEY_MGMT:
		...
		if ((ssid->key_mgmt && WPA_KEY_MGMT_WAPI_PSK)
			...
			sk_intf_data_set_wapi_enable(true);
		}
		else
			sk_intf_data_set_wapi_enable(false);
#endif
		break;
