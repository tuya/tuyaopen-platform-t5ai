...
int wpa_supplicant_event_assoc_wapi_network(struct wpa_supplicant *wpa_s,
		int freq);
...
#if CONFIG_WAPI_SUPPORT
	const u8 *uni_wapi_ie = get_ie(ie_start, len, WLAN_EID_BSS_AC_ACCESS_DELAY);
	if (uni_wapi_ie) {
		if (uni_wapi_ie[9] == 2)
			return WIFI_SECURITY_TYPE_WAPI_PSK;
		else if (uni_wapi_ie[9] == 1)
			return WIFI_SECURITY_TYPE_WAPI_CERT;
		else
			return WIFI_SECURITY_TYPE_WAPI_UNKNOWN;
	}
#endif
			    ...
			    bool debug_print)
{
...
#if CONFIG_WAPI_SUPPORT
	const u8 *wapi_ie = wpa_bss_get_ie(bss, WLAN_EID_BSS_AC_ACCESS_DELAY);
	wpa |= wapi_ie && wapi_ie[1];
#endif
	...
	if (ssid->proto & WPA_PROTO_WAPI) {
		if (wapi_ie==NULL) {
			...
			else if ((ssid->key_mgmt & WPA_KEY_MGMT_WAPI_CERT) && (wapi_ie[9] == 1)) {
					      ...
					      int new_scan, int own_request)
{
	...
	if (selected) {
		...
		if (bk_feature_network_found_event()) {
			...
			BK_LOG_ON_ERR(bk_event_post(EVENT_MOD_WIFI, EVENT_WIFI_NETWORK_FOUND,
...
int wpa_supplicant_event_assoc_wapi_network(struct wpa_supplicant *wpa_s, int freq){
	...
	if (wpa_s->current_ssid && (wpa_s->current_ssid->proto & WPA_PROTO_WAPI) && wpa_s->wapi) {
		...
		if (wpa_drv_get_bssid(wpa_s, bssid) >= 0 &&
			...
			wpa_msg(wpa_s, MSG_DEBUG, "WAPI Associated to a new BSS: BSSID="
				MACSTR, MAC2STR(bssid));
			os_memcpy(wpa_s->bssid, bssid, ETH_ALEN);
			os_memset(wpa_s->pending_bssid, 0, ETH_ALEN);
		}
		...
		wpa_s->wapi->wapi_process_assoc_event(bssid, wpa_s->own_addr, (u8 *)ap_wapi_ie, ap_wapi_ie_len);

		wpa_supplicant_cancel_auth_timeout(wpa_s);
		wpa_supplicant_set_state(wpa_s, WPA_ASSOCIATED);
		wpa_msg(wpa_s, MSG_INFO, "WAPI Associated with " MACSTR, MAC2STR(bssid));
