...
#if CONFIG_WAPI_SUPPORT
int wpa_supplicant_wapi_set_key(struct wpa_supplicant *wpa_s, enum wpa_alg alg,
				   const u8 *addr, int key_idx, int set_tx,
				   const u8 *seq, size_t seq_len,
				   const u8 *key, size_t key_len, enum key_flag flag);
...
#if CONFIG_WAPI_SUPPORT
	if (wpas_start_assoc_wapi_network_cb(wpa_s, ssid, bss, params) != -1)
		return;
#endif
...
#if CONFIG_WAPI_SUPPORT
//extern wapi_lib_t *wapi_load_lib();
//	wpa_s->wapi = wapi_load_lib();
	wpa_s->wapi = NULL;
	if (wpa_s->wapi) {
		...
		wpa_s->wapi->wapi_disconnect_notify = NULL;
		wpa_s->wapi->wapi_set_key = wpa_supplicant_wapi_set_key;

		wpa_s->wapi->eloop_register_timeout = eloop_register_timeout;
		wpa_s->wapi->eloop_cancel_timeout = eloop_cancel_timeout;

		if (wpa_s->wapi->wpa_global_priv) {
			wpa_s->wapi->wpa_global_priv(wpa_s->global);
		}

		if (wpa_debug_level >= MSG_EXCESSIVE && wpa_debug_level <= MSG_ERROR) {
			wpa_s->wapi->set_debug_level(wpa_debug_level);
		} else {
			wpa_s->wapi->set_debug_level(MSG_INFO);
		}
	}
#endif
	...
	struct wpa_ssid *ssid, struct wpa_bss *bss, struct wpa_driver_associate_params params)
{
	...
	if (bss) {
		...
		params.freq.freq = bss->freq;
		params.mode = ssid->mode;
		params.auth_alg = ssid->auth_alg ? ssid->auth_alg : WPA_AUTH_ALG_OPEN;
