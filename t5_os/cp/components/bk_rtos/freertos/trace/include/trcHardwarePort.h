...
#define TRC_PORT_SPECIFIC_INIT() vTraceTimerReset()
	...
	#define TRACE_ENTER_CRITICAL_SECTION() TRACE_ALLOC_CRITICAL_SECTION_NAME = port_disable_interrupts_flag()
    ...
    **************************************************************************/
	...
	#elif CONFIG_AON_RTC || CONFIG_ANA_RTC
		#define TRC_HWTC_TYPE TRC_FREE_RUNNING_32BIT_INCR
		#define TRC_HWTC_COUNT (uint32_t)(bk_aon_rtc_get_us()/1000)
		#define TRC_HWTC_PERIOD 1
		#define TRC_HWTC_DIVISOR 1
		#define TRC_HWTC_FREQ_HZ 1000
		#define TRC_IRQ_PRIORITY_ORDER 0
	#else
    ...
    *****************************************************************************************/	
	...
	uint64_t riscv_get_mtimer(void);
