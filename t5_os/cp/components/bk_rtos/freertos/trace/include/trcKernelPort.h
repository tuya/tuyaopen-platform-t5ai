...
#define traceTASK_INCREMENT_TICK( xTickCount ) \
	if (uxSchedulerSuspended[portGET_CORE_ID()] == ( unsigned portBASE_TYPE ) pdTRUE || xPendedTicks == 0) { trcKERNEL_HOOKS_INCREMENT_TICK(); } \
	if (uxSchedulerSuspended[portGET_CORE_ID()] == ( unsigned portBASE_TYPE ) pdFALSE) { trcKERNEL_HOOKS_NEW_TIME(DIV_NEW_TIME, xTickCount + 1); }
#else
#define traceTASK_INCREMENT_TICK( xTickCount ) \
...
#define traceTASK_DELAY() \
	trcKERNEL_HOOKS_TASK_DELAY(TASK_DELAY, pxCurrentTCBs[portGET_CORE_ID()], xTicksToDelay); \
	trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
#else
#define traceTASK_DELAY() \
...
#define traceTASK_DELAY_UNTIL(xTimeToWake) \
	trcKERNEL_HOOKS_TASK_DELAY(TASK_DELAY_UNTIL, pxCurrentTCBs[portGET_CORE_ID()], xTimeToWake); \
	trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
#else
#define traceTASK_DELAY_UNTIL(xTimeToWake) \
...
#define traceTASK_NOTIFY_TAKE(index) \
	if (pxCurrentTCB->ucNotifyState[index] == taskNOTIFICATION_RECEIVED){ \
		trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TRACE_TASK_NOTIFY_TAKE, TASK, pxCurrentTCBs[portGET_CORE_ID()], xTicksToWait); \
	}else{ \
		trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TRACE_TASK_NOTIFY_TAKE_TRCFAILED, TASK, pxCurrentTCB, xTicksToWait);}
#else
#define traceTASK_NOTIFY_TAKE(index) \
...
#define traceTASK_NOTIFY_TAKE_BLOCK(index) \
	trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TRACE_TASK_NOTIFY_TAKE_TRCBLOCK, TASK, pxCurrentTCBs[portGET_CORE_ID()], xTicksToWait); \
	trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
#else
#define traceTASK_NOTIFY_TAKE_BLOCK(index) \
	trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(TRACE_TASK_NOTIFY_TAKE_TRCBLOCK, TASK, pxCurrentTCB, xTicksToWait); \
...
#define traceTASK_NOTIFY_WAIT_BLOCK(index) \
	if (TRACE_GET_OBJECT_FILTER(TASK, pxCurrentTCBs[portGET_CORE_ID()]) & CurrentFilterMask) \
		prvTraceStoreKernelCallWithParam(TRACE_TASK_NOTIFY_WAIT_TRCBLOCK, TRACE_CLASS_TASK, TRACE_GET_TASK_NUMBER(pxCurrentTCBs[portGET_CORE_ID()]), xTicksToWait); \
	trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
#else
#define traceTASK_NOTIFY_WAIT_BLOCK(index) \
	if (TRACE_GET_OBJECT_FILTER(TASK, pxCurrentTCB) & CurrentFilterMask) \
...
#if CONFIG_FREERTOS_SMP
#define traceTASK_SWITCHED_IN() \
	xTraceTaskSwitch(pxCurrentTCBs[portGET_CORE_ID()], pxCurrentTCBs[portGET_CORE_ID()]->uxPriority)
#else
#define traceTASK_SWITCHED_IN() \
