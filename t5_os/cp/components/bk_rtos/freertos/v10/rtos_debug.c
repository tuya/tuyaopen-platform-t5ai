...
void stack_thread_show(uint32_t *total_cnt, uint32_t *used_cnt)
{
	...
	for (i = 0; i < uxThdCnt; i ++) {
		...
		BK_DUMP_OUT("CPU%d:%-16s   %-10d   0x%-08x- 0x%-08x   %-10d   %-13d   %-11d\r\n",
					pxTaskItem->xCoreID + CPU_ID_OFFSET,
...
void rtos_dump_stack_memory_usage(void)
{
	...
	BK_DUMP_OUT("thread_stack:%d, used:%d, the rest:%d\r\n\r\n", total1, used1, total1 - used1);
	stack_arm_mode_show(&total2, &used2);
	BK_DUMP_OUT("\r\narm_mode_stack:%d, used:%d, the rest:%d\r\n", total2, used2, total2 - used2);
	BK_DUMP_OUT("total_stack_space:%d, used:%d, the rest:%d\r\n", total1 + total2, used1 + used2,
			  total1 + total2 - (used1 + used2));
}
...
void rtos_dump_task_runtime_stats(void)
{
	...
	static inline void rtos_dump_task_history_percentage(char *buf, int buf_len, BK_CpuLoadTime eTime)
	{
		...
		for (size_t i = 0; i < configNUMBER_OF_CORES; i++) {
			BK_DUMP_OUT(">>>>dump cpu%u task\r\n", i);
			vTaskGetHistoryRunTimeStatsByCoreID(buf, eTime, i);
			buf[buf_len - 1] = '\0';
			info_len = strlen(buf);
			BK_DUMP_RAW_OUT(buf, info_len);
		}
...
#if CONFIG_FREERTOS_SMP && ( configUSE_CORE_AFFINITY == 1 ) && ( configNUM_CORES > 1 )
	BK_DUMP_OUT("%-12s   %-5s   %-5s   %-5s   %-5s   %-5s\r\n",
		"task", "state", "pri", "water", "no", "affinity");
#else
