...
static err_t ping_send(int s, ip_addr_t *addr, int size)
{
    ...
    if (g_lwip_dbg_cfg & LWIP_TX_DBG_LOG_PING)
        BK_LOG_RAW("[TX][PING] iecho %x seqno %d\r\n",iecho,iecho->seqno);
    #endif
...
static int ping_recv(int s, int *ttl)
{
    ...
    while ((len = lwip_recvfrom(s, buf, buf_size, 0, (struct sockaddr*) &from, (socklen_t*) &fromlen)) > 0)
    {
        ...
        if (len >= (int)(sizeof(struct ip_hdr) + sizeof(struct icmp_echo_hdr)))
        {
            ...
            #if CONFIG_LWIP_DEBUG_LOG
            if (g_lwip_dbg_cfg & LWIP_RX_DBG_LOG_PING)
                BK_LOG_RAW("[RX][PING] iecho %x seqno %d len %d\r\n",iecho,iecho->seqno,len);
            #endif
...
void ping_start(char* target_name, uint32_t times, size_t size)
{
	if (p_param.state == PING_STATE_STOPPED) {
	    ...
	    if (target_name) {
			...
			rtos_smp_create_thread(NULL, ping_priority, "ping",
