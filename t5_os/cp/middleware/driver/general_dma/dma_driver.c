...
static dma_isr_arg_reg_t s_dma_arg_isr[SOC_DMA_UNIT_NUM][SOC_DMA_CHAN_NUM_PER_UNIT] = {NULL};
...
bk_err_t bk_dma_driver_init(void)
{
    ...
    os_memset(&s_dma_arg_isr, 0, sizeof(s_dma_arg_isr));
...
bk_err_t bk_dma_register_isr_arg(dma_id_t id, dma_isr_arg_reg_t *dma_arg_isr)
{
    uint8_t dma_num,dma_channel;
	dma_id_to_hw_id_ch(id,&dma_num,&dma_channel);
    s_dma_arg_isr[dma_num][dma_channel].half_finish_isr = dma_arg_isr->half_finish_isr;
    s_dma_arg_isr[dma_num][dma_channel].half_finish_arg = dma_arg_isr->half_finish_arg;

    s_dma_arg_isr[dma_num][dma_channel].finish_isr = dma_arg_isr->finish_isr;
    s_dma_arg_isr[dma_num][dma_channel].finish_arg = dma_arg_isr->finish_arg;

    s_dma_arg_isr[dma_num][dma_channel].error_isr = dma_arg_isr->error_isr;
    s_dma_arg_isr[dma_num][dma_channel].error_arg = dma_arg_isr->error_arg;
    return BK_OK;
}

bk_err_t bk_dma_register_half_finish_isr_arg(dma_id_t id, dma_isr_arg_t half_finish_isr, void *arg)
{
    uint8_t dma_num,dma_channel;
	dma_id_to_hw_id_ch(id,&dma_num,&dma_channel);
    s_dma_arg_isr[dma_num][dma_channel].half_finish_isr = half_finish_isr;
    s_dma_arg_isr[dma_num][dma_channel].half_finish_arg = arg;

    return BK_OK;
}

bk_err_t bk_dma_register_finish_isr_arg(dma_id_t id, dma_isr_arg_t finish_isr, void *arg)
{
    uint8_t dma_num,dma_channel;
	dma_id_to_hw_id_ch(id,&dma_num,&dma_channel);
    s_dma_arg_isr[dma_num][dma_channel].finish_isr = finish_isr;
    s_dma_arg_isr[dma_num][dma_channel].finish_arg = arg;

    return BK_OK;
}

bk_err_t bk_dma_register_bus_err_isr_arg(dma_id_t id, dma_isr_arg_t bus_err_isr, void *arg)
{
    uint8_t dma_num,dma_channel;
    dma_id_to_hw_id_ch(id,&dma_num,&dma_channel);
    s_dma_arg_isr[dma_num][dma_channel].error_isr = bus_err_isr;
    s_dma_arg_isr[dma_num][dma_channel].error_arg = arg;
...
//TODO:I think no need to wait copy data finish,just confirm before copy start, the previous one is finish.
    ...
    BK_WHILE(dma_hal_get_enable_status(&s_dma[dma_num].hal, dma_channel))
    {
        ...
        {
            DMA_LOGE("%s! Timeout out:0x%x in:0x%x len:%d cpy_chnl: %d\r\n", __func__, out, in, len, cpy_chnl);
            dma_hal_stop_common(&s_dma[dma_num].hal, dma_channel);
...
static void dma_isr_common(dma_unit_t dma_unit_id)
{
    ...
    uint32_t channel = 0;
    dma_isr_arg_t half_finish_isr_arg = NULL;
    void *half_finish_arg = NULL;
    dma_isr_arg_t finish_isr_arg = NULL;
    void *finish_arg = NULL;
    dma_isr_arg_t error_isr_arg = NULL;
    void *error_arg = NULL;
    ...
    for (int id = 0; id < SOC_DMA_CHAN_NUM_PER_UNIT; id++) {
        ...
        if (dma_hal_is_finish_interrupt_triggered(hal, id)) {
           ...
           DMA_LOGE("dma_isr BUS ERROR TRIGGERED! id: %d\r\n", id);
           if (error_isr_arg) {
               DMA_LOGE("dma_isr bus_err_isr! id: %d\r\n", id);
               dma_hal_clear_finish_interrupt_status(hal, id);
               error_isr_arg(channel, error_arg);
           } else if (s_dma_bus_err_isr[dma_unit_id][id]) {
            DMA_LOGE("dma_isr bus_err_isr! id: %d\r\n", id);
            dma_hal_clear_finish_interrupt_status(hal, id);
