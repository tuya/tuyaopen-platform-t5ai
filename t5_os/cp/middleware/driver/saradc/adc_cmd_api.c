...
{
    const uint16_t cali_val[SARADC_CALIBRATE_MAX1] = {
        #if (CONFIG_SOC_BK7236N || CONFIG_SOC_BK7236Q)
        /*TODO Fixme: wangzhilei*/
        0, 0x1272, 0x23d8 /* 1Volt, 2Volt, 25Degree, 10Step*/
        #elif (CONFIG_SOC_BK7239XX)
        0, 0x1272, 0x23d8 /* 1Volt, 2Volt, 25Degree, 10Step*/
...
float saradc_calculate_test(UINT16 adc_val)
{
    ...
    UINT16 cali_high_val = data->sadc_cali_val[SARADC_CALIBRATE_HIGH1];
...
float bk_adc_data_calculate_test(UINT16 adc_val, UINT8 adc_chan)
{
    ...
    if (NULL == channel_config_ptr) {
        adc_vol_div = ADC_VOL_DIV_NONE;
    } else {
        adc_vol_div = channel_config_ptr->vol_div;
    }
    ...
    cali_value = saradc_calculate_test(adc_val);

    return cali_value;
}

UINT32 saradc_get_calibrate_val(uint16_t *value, SARADC_MODE1 mode)
{
    uint32_t irq_level;
    struct sadc_device *dev = saradc_dev_ptr;
    ...
    if ((mode < SARADC_CALIBRATE_EXT_LOW1) || (SARADC_CALIBRATE_MAX1 <= mode))
    {
