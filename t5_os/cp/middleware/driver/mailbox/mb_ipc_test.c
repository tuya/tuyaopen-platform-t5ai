...
{
	//   cpu0 clients
	CPU0_CLIENT_ID_START_test = IPC_CLIENT_ID_START(0),
	TEST1_CLIENT_ID,
	TEST2_CLIENT_ID,
	TEST3_CLIENT_ID,
...
{
	#if IPC_GET_ID_CPU(TEST_SVR1_ID) !=  MAILBOX_CPU0
	#error server cpu configuration error!
	#endif
	rtos_create_thread(NULL, 3, test_svr_name[0], mb_ipc_test_svr1, 2048, (void *)IPC_GET_ID_PORT(TEST_SVR1_ID));
