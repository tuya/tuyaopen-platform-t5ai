...
#define AUD_FLAC_DEC_CMD_CNT (sizeof(s_aud_flac_dec_commands) / sizeof(struct cli_command))
static const struct cli_command s_aud_flac_dec_commands[] = {
...
int cli_aud_flac_dec_init(void)
{
	return cli_register_commands(s_aud_flac_dec_commands, AUD_FLAC_DEC_CMD_CNT);
}
