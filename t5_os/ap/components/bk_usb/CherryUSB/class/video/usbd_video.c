                    ...
                    if (entity_info->wTerminalType == VIDEO_ITT_CAMERA) {
                        switch (control_selector) {
                            ...
                            case VIDEO_CT_FOCUS_AUTO_CONTROL:
                                ...
                                break;
                            case VIDEO_CT_REGION_OF_INTEREST_CONTROL:
                                switch (setup->bRequest) {
                                    case VIDEO_REQUEST_GET_DEF: {
                                        memset(*data, 0, 10);
                                        *len = 10;
...
#define H264_PAYLOAD_HEADER_LENGTH    0x0c
...
uint32_t usbd_video_h264_payload_fill(uint8_t *input, uint32_t input_len, uint8_t *output, uint32_t *out_len)
{
    ...
    uint32_t last_packet_size;
    uint32_t picture_pos = 0;
    uint32_t max_payload_transfer_size = usbd_video_cfg.probe.dwMaxPayloadTransferSize;

    packets = (input_len % max_payload_transfer_size) ?
    ...
    for (size_t i = 0; i < packets; i++) {
        for (size_t j = 0; j < H264_PAYLOAD_HEADER_LENGTH; j++) {
            output[max_payload_transfer_size * i + j] = uvc_h264_header[j];
        }
        if (i == (packets - 1)) {
            memcpy(&output[H264_PAYLOAD_HEADER_LENGTH + max_payload_transfer_size * i],
                   &input[picture_pos],
                   last_packet_size - H264_PAYLOAD_HEADER_LENGTH);
            output[max_payload_transfer_size * i + 1] |= (1 << 1);
        } else {
            memcpy(&output[H264_PAYLOAD_HEADER_LENGTH + max_payload_transfer_size * i],
                   &input[picture_pos],
                   max_payload_transfer_size - H264_PAYLOAD_HEADER_LENGTH);
    ...
    *out_len = (input_len + H264_PAYLOAD_HEADER_LENGTH * packets);
    return packets;
}
