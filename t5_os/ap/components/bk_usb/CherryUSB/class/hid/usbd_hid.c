...
static uint8_t hid_protocol = 0;
static uint8_t hid_idle_duration = 0;
...
static int hid_class_interface_request_handler(struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
    ...
    switch (setup->bRequest) {
        case HID_REQUEST_GET_REPORT:
            ...
            (*data)[0] = usbd_hid_get_report(intf_num, LO_BYTE(setup->wValue), HI_BYTE(setup->wValue));
            *len = 1;
            break;
...
__WEAK uint8_t usbd_hid_get_report(uint8_t intf, uint8_t report_id, uint8_t report_type)
{
    return 0;
}
...
__WEAK void usbd_hid_set_report(uint8_t intf, uint8_t report_id, uint8_t report_type, uint8_t *report, uint8_t report_len)
{
}

__WEAK void usbd_hid_set_idle(uint8_t intf, uint8_t report_id, uint8_t duration)
{
    hid_idle_duration = duration;
}
