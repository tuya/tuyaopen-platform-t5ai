...
#include <components/cherryusb/usb_scsi.h>
#include "usb_errno.h"
...
static int usbh_msc_scsi_write10(struct usbh_msc *msc_class, uint32_t start_sector, const uint8_t *buffer, uint32_t nsectors)
{
    int nbytes;
    struct CBW *cbw;
...
static int usbh_msc_scsi_read10(struct usbh_msc *msc_class, uint32_t start_sector, const uint8_t *buffer, uint32_t nsectors)
{
    int nbytes;
    struct CBW *cbw;
...
{
	return usbh_msc_scsi_write10(current_msc_class, first_block, dest, block_num);
}

int usbh_device_read(uint32_t first_block, const uint8_t *dest, uint32_t block_num)
{
	return usbh_msc_scsi_read10(current_msc_class, first_block, dest, block_num);
}
...
static int usbh_msc_connect(struct usbh_hubport *hport, uint8_t intf)
{
    ...
    current_msc_class = usb_malloc(sizeof(struct usbh_msc));
    if (current_msc_class == NULL) {
        USB_LOG_ERR("Fail to alloc msc_class\r\n");
        return -ENOMEM;
    }

    memset(current_msc_class, 0, sizeof(struct usbh_msc));
    usbh_msc_devno_alloc(current_msc_class);
    current_msc_class->hport = hport;
    current_msc_class->intf = intf;

    hport->config.intf[intf].priv = current_msc_class;

    ret = usbh_msc_get_maxlun(current_msc_class, g_msc_buf);
    if (ret < 0) {
        return ret;
    }
    ...
    for (uint8_t i = 0; i < hport->config.intf[intf].altsetting[0].intf_desc.bNumEndpoints; i++) {
        ...
        if (ep_desc->bEndpointAddress & 0x80) {
            usbh_hport_activate_epx(&current_msc_class->bulkin, hport, ep_desc);
        } else {
            usbh_hport_activate_epx(&current_msc_class->bulkout, hport, ep_desc);
        }
    }

    ret = usbh_msc_scsi_testunitready(current_msc_class);
    if (ret < 0) {
        USB_LOG_ERR("Fail to scsi_testunitready\r\n");
        return ret;
    }
    ret = usbh_msc_scsi_inquiry(current_msc_class);
    if (ret < 0) {
        USB_LOG_ERR("Fail to scsi_inquiry\r\n");
        return ret;
    }
    ret = usbh_msc_scsi_readcapacity10(current_msc_class);
    if (ret < 0) {
...
void usbh_msc_register()
{
	usbh_register_class_driver(0, (void *)&msc_class_info);
}