...
USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX static uint8_t g_cdc_acm_buf[64];
...
static struct usbh_cdc_acm *usbh_cdc_acm_class_alloc(void)
{
    ...
    for (devno = 0; devno < CONFIG_USBHOST_MAX_CDC_ACM_CLASS; devno++) {
        if ((g_devinuse & (1 << devno)) == 0) {
            g_devinuse |= (1 << devno);
            memset(&g_cdc_acm_class[devno], 0x00, sizeof(struct usbh_cdc_acm));
...
{
    struct usb_endpoint_descriptor *ep_desc; __maybe_unused_var(ep_desc);
    int ret = 0;
	...
	bk_usbh_hub_class_connect_notification(hport, intf, USB_DEVICE_CLASS_CDC);
#endif
    return ret;
}
...
static int usbh_cdc_acm_disconnect(struct usbh_hubport *hport, uint8_t intf)
{
	...
	bk_usbh_hub_class_disconnect_notification(hport, intf, USB_DEVICE_CLASS_CDC);
#endif
...
void usbh_cdc_acm_class_register(void)
{
