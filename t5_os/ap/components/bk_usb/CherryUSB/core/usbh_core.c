...
static void usbh_find_driver_printf_info(uint8_t if_num, uint8_t class,
    ...
    for (index = usbh_class_info_table_begin; index < usbh_class_info_table_end; index++) {
        if ((index->match_flags & (USB_CLASS_MATCH_VENDOR | USB_CLASS_MATCH_PRODUCT | USB_CLASS_MATCH_INTF_CLASS | USB_CLASS_MATCH_INTF_SUBCLASS | USB_CLASS_MATCH_INTF_PROTOCOL)) ==
            ...
            if (index->vid == vid && index->pid == pid &&
                ...
                usbh_find_driver_printf_info(1, index->class, index->subclass, index->protocol, index->vid, index->pid);
            ...
            if (index->class == class && index->subclass == subclass && index->protocol == protocol) {
                usbh_find_driver_printf_info(2, index->class, index->subclass, index->protocol, 0, 0);
                return index->class_driver;
            }
            ...
            if (index->class == class) {
                usbh_find_driver_printf_info(5, index->class, 0, 0, 0, 0);
                return index->class_driver;
            }
        }
    }
...
static void usbh_ep0_request_buffer_clear(void)
{
	memset(ep0_request_buffer, 0x0, sizeof(ep0_request_buffer));
}
...
#define USB_REQUEST_BUFFER_SIZE 256
    ...
    if (ret < 0) {
		...
		if((-ret) == EIO) {
			usbh_ep0_request_buffer_clear();
			ret = usbh_control_transfer(hport->ep0, setup, ep0_request_buffer);
			if (ret < 0) {USB_LOG_ERR("Twice failed to get device descriptor,errorcode:%d\r\n", ret); goto errout;}
		} else {
			goto errout;
		}
    }
...
#endif
    /* Wait device parse descriptor completely */
    usb_osal_msleep(2);
    ...
    if (ret < 0) {
		...
		if((-ret) == EIO) {
			ret = usbh_control_transfer(hport->ep0, setup, NULL);
			if (ret < 0) {USB_LOG_ERR("Twice failed to set devaddr,errorcode:%d\r\n", ret); goto errout;}
		} else {
			goto errout;
		}
    }
    ...
    if (ret < 0) {
		...
		if((-ret) == EIO) {
			usbh_ep0_request_buffer_clear();
			ret = usbh_control_transfer(hport->ep0, setup, ep0_request_buffer);
			if (ret < 0) {USB_LOG_ERR("Twice failed to get full device descriptor,errorcode:%d\r\n", ret); goto errout;}
		} else {
			goto errout;
		}
    }
    ...
    if (ret < 0) {
		...
		if((-ret) == EIO) {
			usbh_ep0_request_buffer_clear();
			ret = usbh_control_transfer(hport->ep0, setup, ep0_request_buffer);
			if (ret < 0) {USB_LOG_ERR("Twice failed to get config descriptor,errorcode:%d\r\n", ret); goto errout;}
		} else {
			goto errout;
		}
    }
    ...
    if (ret < 0) {
		...
		if((-ret) == EIO) {
			usbh_ep0_request_buffer_clear();
			ret = usbh_control_transfer(hport->ep0, setup, ep0_request_buffer);
			if (ret < 0) {USB_LOG_ERR("Twice failed to get full config descriptor,errorcode:%d\r\n", ret); goto errout;}
		} else {
			goto errout;
		}
    }
...
int usbh_control_transfer(usbh_pipe_t pipe, struct usb_setup_packet *setup, uint8_t *buffer)
{
    ...
    if (urb == NULL) {
        ret = -ENOMEM;
        USB_LOG_ERR("No memory to alloc. usbh_control_transfer urb\r\n");
        return ret;
    }
