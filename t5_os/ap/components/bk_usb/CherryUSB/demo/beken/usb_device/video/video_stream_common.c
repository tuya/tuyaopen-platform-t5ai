...
static beken_semaphore_t wait_for_player = NULL;
...
void usbd_video_open(uint8_t intf)
{
    ...
    USB_LOG_RAW("OPEN\r\n");
    iso_tx_busy = false;
	rtos_set_semaphore(&wait_for_player);
}
...
void usbd_video_h264_init()
{
	...
	rtos_init_semaphore_ex(&wait_for_player, 2, 0);
...
const uint8_t h264_buf[12] = {
    0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c,
};

#define USBD_ISO_SIMPLE_TEST    1

static void usbd_video_h264_task_entry(beken_thread_arg_t data)
{
#if USBD_ISO_SIMPLE_TEST
    int ret = 0;
#else
    ...
    while (1) {
        ...
        if (tx_flag) {
            iso_tx_busy = true;
            do {
                ret = usbd_ep_start_write(VIDEO_IN_EP, h264_buf, 12);
            } while(ret);
            while (iso_tx_busy) {
                if (tx_flag == 0) {
                    break;
