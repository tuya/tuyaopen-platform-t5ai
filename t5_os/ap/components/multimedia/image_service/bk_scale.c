...
bk_err_t bk_scale_block_fill(uint8_t type, scale_block_t *block)
{
...
void source_block_complete(scale_result_t *result, scale_block_t *block)
{
    ...
    bk_scale_block_fill(SCALE_SOURCE_LINE_FILL, &g_scale_param.src_block);
...
void dest_block_complete(scale_result_t *result, scale_block_t *block)
{
    ...
    bk_scale_block_fill(SCALE_DEST_LINE_FILL, &g_scale_param.dst_block);
}


bk_err_t scale_handler(scale_param_t *scale_param, frame_buffer_t *src, frame_buffer_t *dst)
{
    bk_err_t ret;
...
bk_err_t bk_hw_scale(frame_buffer_t *src, frame_buffer_t *dst)
{
    bk_err_t ret = BK_FAIL;

    scale_handler(&g_scale_param, src, dst);
