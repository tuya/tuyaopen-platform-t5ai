...
static sw_dec_config_t *sw_dec_config_cp2 = NULL;
...
jpeg_dec_handle_t jpeg_decode_cp2_handle;

bk_err_t software_decode_minor_task_send_msg(uint32_t type, uint32_t param)
{
	int ret = BK_OK;
	media_msg_t msg;

	if (sw_dec_config_cp2 && sw_dec_config_cp2->sw_dec_queue)
	{
		msg.event = type;
		msg.param = param;

		ret = rtos_push_to_queue(&sw_dec_config_cp2->sw_dec_queue, &msg, BEKEN_WAIT_FOREVER);
...
static void software_decode_task_deinit(void)
{
	...
	if (sw_dec_config_cp2)
	{
		...
		{
			rtos_deinit_queue(&sw_dec_config_cp2->sw_dec_queue);
			sw_dec_config_cp2->sw_dec_queue = NULL;
		}
		if(sw_dec_config_cp2->sw_dec_sem)
		{
			rtos_deinit_semaphore(&sw_dec_config_cp2->sw_dec_sem);
			sw_dec_config_cp2->sw_dec_sem = NULL;
		}
#if CONFIG_SOFTWARE_DECODE_SRAM_MAPPING
		sw_dec_config_cp2->rotate_buffer = NULL;
#else
		if (sw_dec_config_cp2->rotate_buffer)
		{
			os_free(sw_dec_config_cp2->rotate_buffer);
			sw_dec_config_cp2->rotate_buffer = NULL;
		}
#endif
		sw_dec_config_cp2->sw_dec_thread = NULL;
...
#if CONFIG_SOFTWARE_DECODE_SRAM_MAPPING
__attribute__((section(".itcm_sec_code "))) bk_err_t software_cpu2_copy(uint8_t *out, uint8_t *in,
...
static bk_err_t software_decode_frame(frame_buffer_t *in_frame, frame_buffer_t *out_frame)
{
	...
	if (sw_dec_config_cp2->rotate_angle != ROTATE_NONE)
	{
#if CONFIG_SOFTWARE_DECODE_SRAM_MAPPING
		if (sw_dec_config_cp2->rotate_buffer == NULL)
		{
			sw_dec_config_cp2->rotate_buffer = rotate_buffer_sw_cp2;
		}
#else
		if (sw_dec_config_cp2->rotate_buffer == NULL)
		{
			sw_dec_config_cp2->rotate_buffer = os_malloc(16 * 16 * 2);
			if (sw_dec_config_cp2->rotate_buffer == NULL)
			{
	...
	jd_set_rotate_by_handle(jpeg_decode_cp2_handle, sw_dec_config_cp2->rotate_angle, sw_dec_config_cp2->rotate_buffer);
	if (sw_dec_config_cp2->rotate_angle == ROTATE_90 || sw_dec_config_cp2->rotate_angle == ROTATE_270)
	{
		out_frame->width = in_frame->height;
		out_frame->height = in_frame->width;
	}
	FLUSH_ALL_DCACHE();
	ret = bk_jpeg_dec_sw_start_by_handle(jpeg_decode_cp2_handle, JPEGDEC_BY_FRAME, in_frame->frame, out_frame->frame,
...
{
	int ret = BK_OK;
	sw_dec_config_cp2->task_state = true;
	...
	{
		media_msg_t msg;
		ret = rtos_pop_from_queue(&sw_dec_config_cp2->sw_dec_queue, &msg, BEKEN_WAIT_FOREVER);
		if (ret == BK_OK)
		{
			switch (msg.event)
			{
				...
				case EVENT_JPEG_DEC_SET_ROTATE_ANGLE_NOTIFY:
				{
					...
					{
						sw_dec_config_cp2->rotate_angle = rotate_angle;
					}
				}
				break;
...
bool check_software_decode_minor_task_is_open(void)
{
	if (sw_dec_config_cp2 == NULL)
	{
		return false;
	}
	else
	{
		return sw_dec_config_cp2->task_state;
	}
}

void software_decode_minor_set_rotate(uint8_t rotate_angle)
{
	software_decode_minor_task_send_msg(EVENT_JPEG_DEC_SET_ROTATE_ANGLE_NOTIFY, rotate_angle);
}

static void create_thread_on_cp2(beken_thread_arg_t data)
{
#if CONFIG_SOFTWARE_DECODE_SRAM_MAPPING
	sw_dec_config_cp2->sw_dec_thread = xTaskCreateStaticPinnedToCore( (TaskFunction_t)software_decode_main,
										"sw_dec_task_cp2 smp",
										512,
										( void * ) NULL,
										...
										&xSWDecTaskTCB_cp2,
										1);
	if (sw_dec_config_cp2->sw_dec_thread == NULL)
	{
		LOGE("%s %d create thread sw_dec_task_cp2 failed\r\n", __func__, __LINE__);
		rtos_set_semaphore(&sw_dec_config_cp2->sw_dec_sem);
	}
#else
	int ret = BK_OK;
    ret = rtos_core1_create_thread(&sw_dec_config_cp2->sw_dec_thread,
	...
	if (ret != BK_OK)
	{
		LOGE("Error: Failed to create sw_dec_task_cp2: %d\r\n",ret);
		rtos_set_semaphore(&sw_dec_config_cp2->sw_dec_sem);
	}
#endif
	rtos_delete_thread(NULL);
...
bk_err_t software_decode_minor_task_open(uint32_t rotate_buffer)
{
	...
	sw_dec_config_cp2 = (sw_dec_config_t *)os_malloc(sizeof(sw_dec_config_t));
	if (sw_dec_config_cp2 == NULL)
	{
		LOGE("%s, malloc sw_dec_config_cp2 failed\r\n", __func__);
		return BK_FAIL;
	}

	os_memset(sw_dec_config_cp2, 0, sizeof(sw_dec_config_t));



	ret = rtos_init_semaphore(&sw_dec_config_cp2->sw_dec_sem, 1);
	if (ret != BK_OK)
	{
		LOGE("%s, init sw_dec_config_cp2->sw_dec_sem failed\r\n", __func__);
		goto error;
	}

	ret = rtos_init_queue(&sw_dec_config_cp2->sw_dec_queue,
							"sw_dec_que_cp2",
							sizeof(media_msg_t),
							15);
	...
	rtos_get_semaphore(&sw_dec_config_cp2->sw_dec_sem, BEKEN_NEVER_TIMEOUT);
...
bk_err_t software_decode_minor_task_close()
{
	...
	sw_dec_config_cp2->task_state = false;

	software_decode_minor_task_send_msg(JPEGDEC_STOP, 0);
