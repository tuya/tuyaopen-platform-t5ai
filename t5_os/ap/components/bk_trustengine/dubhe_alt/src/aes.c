...
{
    mbedtls_aes_init( &ctx->MBEDTLS_PRIVATE(crypt) );
    mbedtls_aes_init( &ctx->MBEDTLS_PRIVATE(tweak) );
}
...
void dubhe_aes_xts_free(mbedtls_aes_xts_context *ctx)
{
    ...
    mbedtls_aes_free(&ctx->MBEDTLS_PRIVATE(crypt));
    mbedtls_aes_free(&ctx->MBEDTLS_PRIVATE(tweak));
}
                                ...
                                unsigned int keybits)
{
    ...
    ret = dubhe_aes_setkey_enc(&ctx->MBEDTLS_PRIVATE(tweak), key2, key2bits);
    if (ret != 0) {
        return ret;
    }

    /* Set crypt key for encryption. */
    return dubhe_aes_setkey_enc(&ctx->MBEDTLS_PRIVATE(crypt), key1, key1bits);
}
                                ...
                                unsigned int keybits)
{
    ...
    return dubhe_aes_setkey_dec(&ctx->MBEDTLS_PRIVATE(crypt), key1, key1bits);
}
                           ...
                           unsigned char *output )
{
    ...
    /* Compute the tweak. */
    ret = dubhe_aes_crypt_ecb( &ctx->MBEDTLS_PRIVATE(tweak), MBEDTLS_AES_ENCRYPT,
                                 data_unit, tweak );
    if( ret != 0 )
        return( ret );
    ...
    while( blocks-- )
    {
        ...
        ret = dubhe_aes_crypt_ecb( &ctx->MBEDTLS_PRIVATE(crypt), mode, tmp, tmp );
        if( ret != 0 )
            return( ret );
