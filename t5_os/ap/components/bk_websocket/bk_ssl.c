...
#define TAG "SSL"
#define CONFIG_BK_TLS_SKIP_SERVER_CERT_VERIFY 1

/*-------------------------------mbedtls--------------------------------------*/
#define _bk_create_ssl_handle			   bk_ws_create_mbedtls_handle
#define _bk_tls_handshake				   bk_ws_mbedtls_handshake
#define _bk_tls_read					   bk_ws_mbedtls_read
#define _bk_tls_write					   bk_ws_mbedtls_write
#define _bk_tls_conn_delete 			   bk_ws_mbedtls_conn_delete
#define _bk_tls_net_init				   bk_ws_mbedtls_net_init
#define _bk_tls_get_bytes_avail 		   bk_ws_mbedtls_get_bytes_avail

static mbedtls_x509_crt *global_cacert = NULL;

ssize_t bk_ws_mbedtls_get_bytes_avail(bk_tls_t *tls)
{
	if (!tls) {
...
void bk_ws_mbedtls_cleanup(bk_tls_t *tls)
{
	if (!tls) {
		return;
	}
...
static bk_err_t set_ca_cert(bk_tls_t *tls, const unsigned char *cacert, size_t cacert_len)
{
	...
	if (ret == 0)
		BK_LOGV(TAG, "mbedtls_x509_crt_parse returned 0 \r\n");
...
#endif
	BK_LOGV(TAG, "mbedtls_ssl_conf_ca_chain\r\n");
	mbedtls_ssl_conf_ca_chain(&tls->conf, tls->cacert_ptr, NULL);
	return BK_OK;
}
...
static bk_err_t set_client_config(const char *hostname, size_t hostlen, bk_tls_cfg_t *cfg, bk_tls_t *tls)
{
	int ret;
	if (!cfg->skip_common_name) {
		...
		BK_LOGV(TAG, "mbedtls_ssl_set_hostname use_host: %s\r\n", use_host);
	...
	BK_LOGV(TAG, "mbedtls_ssl_config_defaults\r\n");
	...
	BK_LOGV(TAG, "mbedtls_ssl_conf_renegotiation\r\n");
	...
	if (cfg->crt_bundle_attach != NULL) {
		...
		BK_LOGV(TAG, "mbedtls_ssl_conf_authmode\r\n");
		mbedtls_ssl_conf_authmode(&tls->conf, MBEDTLS_SSL_VERIFY_REQUIRED);
	...
	if (cfg->use_secure_element) {
		...
		BK_LOGV(TAG, "clientcert_pem_buf\r\n");
		bk_tls_pki_t pki = {
			.public_cert = &tls->clientcert,
			.pk_key = &tls->clientkey,
			.publiccert_pem_buf = cfg->clientcert_buf,
...
bk_err_t bk_ws_create_mbedtls_handle(const char *hostname, size_t hostlen, const void *cfg, bk_tls_t *tls)
{
	...
	BK_LOGV(TAG, "mbedtls_ctr_drbg_seed\r\n");
#if CONFIG_PSA_MBEDTLS
...
int bk_ws_mbedtls_handshake(bk_tls_t *tls, const bk_tls_cfg_t *cfg)
{
	int ret;
	...
	BK_LOGI(TAG, "    [ Maximum outgoing record payload length is %u ]\n",
		...
		if (ret != BK_TLS_ERR_SSL_WANT_READ && ret != BK_TLS_ERR_SSL_WANT_WRITE) {
			...
			if (cfg->cacert_buf != NULL || cfg->use_global_ca_store == true) {
				...
				bk_ws_mbedtls_verify_certificate(tls);
			}
...
ssize_t bk_ws_mbedtls_read(bk_tls_t *tls, char *data, size_t datalen)
{
...
ssize_t bk_ws_mbedtls_write(bk_tls_t *tls, const char *data, size_t datalen)
{
	size_t written = 0;
	size_t write_len = datalen;
	BK_LOGV(TAG, "bk_mbedtls_write, write_len:%d\r\n", write_len);
	while (written < datalen) {
		...
		if (ret <= 0) {
			...
			BK_LOGV(TAG, "mbedtls_ssl_write, ret:%d\r\n", ret);
		written += ret;
		write_len = datalen - written;
	}
	BK_LOGV(TAG, "bk_mbedtls_write, written:%d\r\n", written);
	return written;
}

void bk_ws_mbedtls_conn_delete(bk_tls_t *tls)
{
	if (tls != NULL) {
		bk_ws_mbedtls_cleanup(tls);
		if (tls->is_tls) {
...
static inline void bk_ws_mbedtls_net_init(bk_tls_t *tls)
{
...
static bk_err_t bk_tls_set_socket_options(int fd, const bk_tls_cfg_t *cfg)
{
	if (cfg) {
		...
		if (cfg->keep_alive_cfg && cfg->keep_alive_cfg->keep_alive_enable) {
			...
			BK_LOGV(TAG, "Enable TCP keep alive. idle: %d, interval: %d, count: %d\r\n", keep_alive_idle, keep_alive_interval, keep_alive_count);
		...
		if (cfg->if_name) {
			if (cfg->if_name->ifr_name[0] != 0) {
				BK_LOGV(TAG, "Bind [sock=%d] to interface %s\r\n", fd, cfg->if_name->ifr_name);
...
static ssize_t bk_tls_get_bytes_avail(bk_tls_t *tls)
{
	return _bk_tls_get_bytes_avail(tls);
}

static int bk_tls_conn_destroy(bk_tls_t *tls)
{
	if (tls != NULL) {
		int ret = 0;
	...
	if (connect(fd, (struct sockaddr *)&address, sizeof(struct sockaddr)) < 0) {
		if (errno == EINPROGRESS) {
			...
			if (res < 0) {
				...
				BK_LOGE(TAG, "[sock=%d] select() timeout error: %d\r\n", fd, (errno));
				goto err;
			} else {
				int sockerr;
...
static int tls_low_level_conn(const char *hostname, int hostlen, int port, const bk_tls_cfg_t *cfg, bk_tls_t *tls)
{
	...
	case BK_TLS_INIT:
		...
		if ((bk_ret = tls_tcp_connect(hostname, hostlen, port, cfg, &tls->sockfd)) != BK_OK) {
			BK_LOGE(TAG, "non-tls connection fail\r\n");
			return -1;
		}
		if (tls->is_tls == false) {
...
static int tls_conn_new_sync(const char *hostname, int hostlen, int port, const bk_tls_cfg_t *cfg, bk_tls_t *tls)
{
...
/*---------------------------------ssl-------------------------------------*/
static struct timeval* bk_transport_utils_ms_to_timeval(int timeout_ms, struct timeval *tv)
{
	if (timeout_ms == -1) {
		return NULL;
	}
...
int _ssl_base_poll_read(transport_bk_tls_t *ssl, int timeout_ms)
{
	int ret = -1;
	int remain = 0;
...
int _ssl_base_poll_write(transport_bk_tls_t *ssl, int timeout_ms)
{
	int ret = -1;
	struct timeval timeout;
	fd_set writeset;
...
int _ssl_connect(transport_bk_tls_t *ssl, const char *host, int port, int timeout_ms)
{
...
int _ssl_tcp_connect(transport_bk_tls_t *ssl, const char *host, int port, int timeout_ms)
{
	bk_err_t ret;
...
int _ssl_write(transport_bk_tls_t *ssl, const char *buffer, int len, int timeout_ms)
{
	int poll;
	BK_LOGV(TAG, "ssl_write\r\n");
	if ((poll = _ssl_base_poll_write(ssl, timeout_ms)) <= 0) {
		BK_LOGE(TAG, "Poll timeout or error, errno=%s, fd=%d, timeout_ms=%d\r\n", strerror(errno), ssl->sockfd, timeout_ms);
		return poll;
	}
	else
		BK_LOGV(TAG, "ssl_base_poll_write, poll=%d, poll success\r\n", poll);
	int ret = _bk_tls_write(ssl->tls, (const char *) buffer, len);
	if (ret < 0) {
		BK_LOGE(TAG, "bk_tls_conn_write error, errno=%s errno:%d\r\n", strerror(errno), errno);
	}
	BK_LOGV(TAG, "ssl_write ret:%d\r\n", ret);
	return ret;
}

int _ssl_tcp_write(transport_bk_tls_t *ssl, const char *buffer, int len, int timeout_ms)
{
	int ret, poll;

	BK_LOGV(TAG, "tcp_write\r\n");
	if ((poll = _ssl_base_poll_write(ssl, timeout_ms)) <= 0) {
		BK_LOGE(TAG, "Poll timeout or error, errno=%s, fd=%d, timeout_ms=%d\r\n", strerror(errno), ssl->sockfd, timeout_ms);
		return poll;
	} else {
...
int _ssl_read(transport_bk_tls_t *ssl, char *buffer, int len, int timeout_ms)
{
	int poll;
	BK_LOGV(TAG, "ssl_read, len:%d\r\n", len);
	if ((poll = _ssl_base_poll_read(ssl, timeout_ms)) <= 0) {
	...
	if(ret > 0)
		BK_LOGV(TAG, "_bk_tls_read, ret:%d\r\n", ret);
	return ret;
}

int _ssl_tcp_read(transport_bk_tls_t *ssl, char *buffer, int len, int timeout_ms)
{
	int ret, poll;

	BK_LOGV(TAG, "tcp_read, len:%d\r\n", len);
	if ((poll = _ssl_base_poll_read(ssl, timeout_ms)) <= 0) {
		BK_LOGE(TAG, "ssl_base_poll_read fail\r\n");
		return poll;
	}
...
int _ssl_base_close(transport_bk_tls_t *ssl)
{
	int ret = -1;
