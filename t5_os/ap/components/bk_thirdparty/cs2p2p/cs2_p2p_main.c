...
#define CHECK_WRITE_THRESHOLD_SIZE  (1024 * 32) //(1024 * 256)
...
static void get_current_time(st_time_info *pt)
{
    ...
    struct cs2_tm *ptm = localtime_r((const cs2_time_t *)&tmv.tv_sec, &stm);
...
static int cs2_get_sinfo(int SessionID, st_session_info *InputSInfo)
{
    memset(InputSInfo, 0, sizeof(st_session_info));
    st_PPCS_Session Sinfo;
    int ret = PPCS_Check(SessionID, &Sinfo);

    if (ERROR_PPCS_SUCCESSFUL == ret)
    {
        InputSInfo->Skt = Sinfo.Skt;
        // Remote addr
        snprintf(InputSInfo->RemoteIP, sizeof(InputSInfo->RemoteIP), "%s", inet_ntoa(Sinfo.RemoteAddr.sin_addr));
        InputSInfo->RemotePort = ntohs(Sinfo.RemoteAddr.sin_port);
        // Lan addr
        snprintf(InputSInfo->LocalIP, sizeof(InputSInfo->LocalIP), "%s", inet_ntoa(Sinfo.MyLocalAddr.sin_addr));
        InputSInfo->LocalPort = ntohs(Sinfo.MyLocalAddr.sin_port);
        // Wan addr
        snprintf(InputSInfo->WanIP, sizeof(InputSInfo->WanIP), "%s", inet_ntoa(Sinfo.MyWanAddr.sin_addr));
        InputSInfo->WanPort = ntohs(Sinfo.MyWanAddr.sin_port);

        InputSInfo->ConnectTime = Sinfo.ConnectTime;
        memcpy(InputSInfo->DID, Sinfo.DID, strlen(Sinfo.DID));
        InputSInfo->bCorD = Sinfo.bCorD;

        if (0 == Sinfo.bMode)
        {
            if (Sinfo.RemoteAddr.sin_addr.s_addr == Sinfo.MyLocalAddr.sin_addr.s_addr || 1 == is_lan_cmp(InputSInfo->LocalIP, InputSInfo->RemoteIP))
            {
                InputSInfo->bMode = 0;
                1 == get_socket_type(Sinfo.Skt) ? memcpy(InputSInfo->Mode, "LAN.", 4) : memcpy(InputSInfo->Mode, "LAN", 3);
            }
            else { InputSInfo->bMode = 1; memcpy(InputSInfo->Mode, "P2P", 3); }
        }
        else if (1 == Sinfo.bMode) { InputSInfo->bMode = 2; memcpy(InputSInfo->Mode, "RLY", 3); }
        else if (2 == Sinfo.bMode) { InputSInfo->bMode = 3; memcpy(InputSInfo->Mode, "TCP", 3); }
        else if (3 == Sinfo.bMode) { InputSInfo->bMode = 4; memcpy(InputSInfo->Mode, "RP2P", 4); }  //// support by P2P API 5.0.8
    }
...
static int32_t cs2_p2p_send_raw_internal(int SessionID, uint8_t Channel, uint8_t *buff, uint32_t size)
{
    ...
    const uint32_t write_not_send_thr = CHECK_WRITE_THRESHOLD_SIZE;
    ...
    while (*is_run)
    {
        ...
        else
        {
            ...
            if (ERROR_PPCS_SUCCESSFUL == ret)
            {
                ...
                char LanIP[16] = {};
                snprintf(LanIP, sizeof(LanIP), "%s", inet_ntoa(Sinfo.MyLocalAddr.sin_addr));
                const char *pMode = NULL;

                if (0 == Sinfo.bMode)
                {
                    if (Sinfo.RemoteAddr.sin_addr.s_addr == Sinfo.MyLocalAddr.sin_addr.s_addr || 1 == is_lan_cmp(LanIP, RmtIP))
                    {
                ...
                st_info("My Lan Addr : %s:%d\n", LanIP, ntohs(Sinfo.MyLocalAddr.sin_port));
