...
int mbedtls_server_start(MbedTLSSessionServer *session, void *entropy, size_t entropyLen)
{
    ...
    ret =  mbedtls_pk_parse_key( &session->pkey, (const unsigned char *) mbedtls_test_srv_key, mbedtls_test_srv_key_len,
                                 NULL, 0, mbedtls_ctr_drbg_random, &session->ctr_drbg);
    if ( ret != 0 ) {
