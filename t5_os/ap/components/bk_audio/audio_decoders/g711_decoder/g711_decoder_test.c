...
#define INPUT_VAL  0xD5
#define OUTPUT_VAL  0x0008
...
static int16_t *input_temp_data = NULL;
static uint8_t *output_temp_data = NULL;
...
static int _el_read(audio_element_handle_t self, char *buffer, int len, TickType_t ticks_to_wait, void *context)
{
    ...
    if (input_temp_data)
    {
        os_memcpy(buffer, input_temp_data, len);
    }
...
static int _el_write(audio_element_handle_t self, char *buffer, int len, TickType_t ticks_to_wait, void *context)
{
    ...
    if (output_temp_data)
    {
        ...
        {
            os_memcpy(output_temp_data, buffer, OUTPUT_RINGBUF_SIZE);
        }
        else
        {
            os_memcpy(output_temp_data, buffer, len);
        }
    }

    /* check the audio data in output buffer */
    int16_t *ptr = (int16_t *)output_temp_data;
...
/* The "g711-decoder" element is neither a producer nor a consumer when test element
  ...
  ...            src - ringbuf - sink           src - ringbuf - sink           ...
   ...
   Function: Use g711 decoder to decode fixed audio data in ringbuffer.

   The "g711-decoder" element read audio data from ringbuffer, decode the data to pcm
...
{"stream_in", "g711_dec", "stream_out"
    ...
    os_free(output_temp_data);
    output_temp_data = NULL;
