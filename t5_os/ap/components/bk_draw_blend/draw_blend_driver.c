#include <stdlib.h>
#include <common/bk_include.h>
#include <os/mem.h>
#include <os/str.h>
#include <driver/dma2d.h>
#include "modules/image_scale.h"
#if  CONFIG_BLEND
#include "modules/lcd_font.h"
#endif
#include "bk_draw_blend.h"

#define TAG "blend_drv"


#define LOGI(...) BK_LOGI(TAG, ##__VA_ARGS__)
#define LOGW(...) BK_LOGW(TAG, ##__VA_ARGS__)
#define LOGE(...) BK_LOGE(TAG, ##__VA_ARGS__)
#define LOGD(...) BK_LOGD(TAG, ##__VA_ARGS__)
#define LOGV(...) BK_LOGV(TAG, ##__VA_ARGS__)


typedef struct {
    uint8_t *addr;
    uint32_t size;
    bool is_malloc_psram; ///0:sram, 1:psram
}blend_malloc_t;

typedef struct
{
	beken_semaphore_t dma2d_complete_sem;
	beken_semaphore_t dma2d_err_sem;
    uint8_t state;
    blend_malloc_t buf1;
    blend_malloc_t buf2;
}blend_t;

static blend_t s_blend = {0};


#if (USE_DMA2D_BLEND_ISR_CALLBACKS == 1)
static void dma2d_config_error(void)
{
	LOGE("%s \n", __func__);
	rtos_set_semaphore(&s_blend.dma2d_err_sem);
}

static void dma2d_transfer_error(void)
{
	LOGE("%s \n", __func__);
	rtos_set_semaphore(&s_blend.dma2d_err_sem);
}

static void dma2d_transfer_complete(void)
{
	LOGV("%s \n", __func__);
	rtos_set_semaphore(&s_blend.dma2d_complete_sem);
}
#endif

static bk_err_t blend_check_mem(uint32_t icon_x, uint32_t icon_y, uint8_t icon_rotate)
{
    if (icon_x * icon_y * 2 > s_blend.buf1.size)
    {
        s_blend.buf1.size = icon_x * icon_y *2;
 
        if (s_blend.buf1.addr == NULL)
        {
            #if BLEND_MALLOC_SRAM
            //first malloc in sram by default if BLEND_MALLOC_SRAM=1
            s_blend.buf1.addr = (uint8_t *)os_malloc(s_blend.buf1.size);
            s_blend.buf1.is_malloc_psram = 0;
            if (s_blend.buf1.addr == NULL)
            #endif
            {
                s_blend.buf1.addr = (uint8_t *)psram_malloc(s_blend.buf1.size);
                s_blend.buf1.is_malloc_psram = 1;
            }
        }
        else
        {
            os_free(s_blend.buf1.addr);
            s_blend.buf1.addr = NULL;
            #if BLEND_MALLOC_SRAM
            //first malloc in sram by default if BLEND_MALLOC_SRAM=1
            s_blend.buf1.addr = (uint8_t *)os_malloc(s_blend.buf1.size);
            s_blend.buf1.is_malloc_psram = 0;
            if (s_blend.buf1.addr == NULL)
            #endif
            {
                s_blend.buf1.addr = (uint8_t *)psram_malloc(s_blend.buf1.size);
                s_blend.buf1.is_malloc_psram = 1;
            }
        }
        if (s_blend.buf1.addr == NULL)
        {
            LOGE("%s malloc ICON size %d*%d*2 %d in %d(0:sram, 1:psram) error\n", __func__, icon_x, icon_y, s_blend.buf1.size, s_blend.buf1.is_malloc_psram );
            return BK_FAIL;
        }
        LOGD("%s malloc ICON size %d*%d*2 %d in %d(0:sram, 1:psram) %p-%p\n", __func__, icon_x, icon_y, s_blend.buf1.size, s_blend.buf1.is_malloc_psram, s_blend.buf1.addr, (char*)(s_blend.buf1.addr + s_blend.buf1.size));

        if (icon_rotate == ROTATE_270 )
        {
            s_blend.buf2.size = icon_x * icon_y *2;
            s_blend.buf2.addr = (uint8_t *)psram_malloc(s_blend.buf2.size);
            if (s_blend.buf2.addr == NULL)
            {
                LOGE("%s malloc buf2 in psram error\n", __func__);
                return BK_FAIL;
            }
            s_blend.buf2.is_malloc_psram = 1;
        }
    }

    return BK_OK;
}

/**
 * @brief This API used to direct draw fg argb8888 png raw data to bg yuv data
 */
bk_err_t bk_image_blend(image_blend_cfg_t *cfg)
{
#if CONFIG_BLEND 
    if (blend_check_mem(cfg->xsize, cfg->ysize, cfg->blend_rotate) != BK_OK)
    {
        return BK_FAIL;
    }

    int i = 0;
    uint16_t lcd_start_x = 0;
    uint16_t lcd_start_y = 0;
    uint8_t pixel_bit;
    uint8_t *p_yuv_src_temp = NULL;
    uint8_t *p_yuv_src = p_yuv_src_temp;
    uint8_t *p_yuv_dst = (uint8_t *)s_blend.buf1.addr;
    uint8_t *p_yuv_temp = (uint8_t *)s_blend.buf2.addr;
    pixel_format_t bg_fmt  = cfg->bg_data_format;

    if ((cfg->lcd_width < cfg->bg_width) || (cfg->lcd_height < cfg->bg_height)) //for lcd size is small then frame image size
    {
        if (cfg->lcd_width < cfg->bg_width)
        {
            lcd_start_x = (cfg->bg_width - cfg->lcd_width) / 2;
        }
        if (cfg->lcd_height < cfg->bg_height)
        {
            lcd_start_y = (cfg->bg_height - cfg->lcd_height) / 2;
        }
    }
    if (cfg->bg_data_format == PIXEL_FMT_RGB888)
    {
        pixel_bit = 3;
    }
    else
    {
        pixel_bit = 2;
    }
    p_yuv_src_temp = (uint8_t *)(cfg->pbg_addr + (((lcd_start_y + cfg->ypos) * cfg->bg_width) + (lcd_start_x + cfg->xpos)) * pixel_bit);
    p_yuv_src = p_yuv_src_temp;

    //STEP 1 COPY BG YUV DATA

    for (i = 0; i < cfg->ysize; i++)
    {
        os_memcpy((uint32_t *)p_yuv_dst, (uint32_t *) p_yuv_src, cfg->xsize * pixel_bit);
        p_yuv_dst += (cfg->xsize * pixel_bit);
        p_yuv_src += (cfg->bg_width * pixel_bit);
    }
    p_yuv_dst = (uint8_t *)s_blend.buf1.addr;

    if (cfg->blend_rotate == ROTATE_270)
    {
        p_yuv_dst = (uint8_t *)s_blend.buf1.addr;
        if (PIXEL_FMT_VUYY == bg_fmt)
        {
            //step2 rotate area
            vuyy_rotate_degree90_to_yuyv((unsigned char *)p_yuv_dst, (unsigned char *)p_yuv_temp, cfg->xsize, cfg->ysize);
        }
        if (PIXEL_FMT_YUYV == bg_fmt)
        {
            yuyv_rotate_degree90_to_yuyv((unsigned char *)p_yuv_dst, (unsigned char *)p_yuv_temp, cfg->xsize, cfg->ysize);
        }

        i = cfg->xsize;
        cfg->xsize = cfg->ysize;
        cfg->ysize = i;
        bg_fmt = PIXEL_FMT_YUYV;
        p_yuv_dst = s_blend.buf2.addr;
    }

    //STEP 2 check alpha=0 logo pixel,and copy alpha!=0 pixel to bg yuv data
    //  if (cfg->flag == 1)
    //  {
    //      p_yuv_temp = s_blend.buf1.addr;
    //      os_memcpy((uint32_t *)(p_yuv_temp), (uint32_t *)cfg->pfg_addr, cfg->xsize * cfg->ysize *4);
    //  }

    if (PIXEL_FMT_VUYY == bg_fmt)
    {
        argb8888_to_vuyy_blend((uint8_t *)cfg->pfg_addr, p_yuv_dst, cfg->xsize, cfg->ysize);
    }
    else if (PIXEL_FMT_YUYV == bg_fmt)
    {
        argb8888_to_yuyv_blend((uint8_t *)cfg->pfg_addr, p_yuv_dst, cfg->xsize, cfg->ysize);
    }
    else if (PIXEL_FMT_RGB888 == bg_fmt)
    {
        argb8888_to_rgb888_blend((uint8_t *)cfg->pfg_addr, p_yuv_dst, cfg->xsize, cfg->ysize);
    }
    else
    {
        argb8888_to_rgb565_blend((uint8_t *)cfg->pfg_addr, p_yuv_dst, cfg->xsize, cfg->ysize);
    }

    if (cfg->blend_rotate == ROTATE_270)
    {
        p_yuv_temp = s_blend.buf1.addr;

        if (PIXEL_FMT_VUYY == cfg->bg_data_format)
        {
            // rotate back
            yuyv_rotate_degree270_to_vuyy((unsigned char *)p_yuv_dst, (unsigned char *)p_yuv_temp, cfg->xsize, cfg->ysize);
        }
        else
        {
            yuyv_rotate_degree270_to_yuyv((unsigned char *)p_yuv_dst, (unsigned char *)p_yuv_temp, cfg->xsize, cfg->ysize);
        }
        i = cfg->xsize;
        cfg->xsize = cfg->ysize;
        cfg->ysize = i;
        p_yuv_dst = s_blend.buf1.addr;
    }

    //STEP 3 copy return bg image
    p_yuv_src = (uint8_t *)p_yuv_src_temp;

    for (i = 0; i < cfg->ysize; i++)
    {
        os_memcpy((uint32_t *)p_yuv_src, (uint32_t *)p_yuv_dst, cfg->xsize * pixel_bit);
        p_yuv_dst += (cfg->xsize * pixel_bit);
        p_yuv_src += (cfg->bg_width * pixel_bit);
    }
#endif
    return BK_OK;
}



/**
 * @brief This API use dma2d to blend fg argb8888 png raw data to bg rgb565 or yuv data
 *
 * 1:in BK7258 V4 version dma2d not spport yuv to rgb convert, so 
 *                if bg is yuv we should use dma2d memcpy yuv and then use software convert to rgb565. 
 * 2:but in BK7258 V5, dma2d support yuv_2_rgb convert, So we direct make fg argb8888 blend with fg yuv data.
 *   users just concern V5
 */
bk_err_t bk_dma2d_image_blend(image_blend_cfg_t *cfg)
{
#if CONFIG_LCD_DMA2D_BLEND
    if (blend_check_mem(cfg->xsize, cfg->ysize, cfg->blend_rotate) != BK_OK)
    {
        return BK_FAIL;
    }

    uint16_t lcd_start_x = 0;
    uint16_t lcd_start_y = 0;
    if ((cfg->lcd_width < cfg->bg_width)  || (cfg->lcd_height < cfg->bg_height)) //for lcd size is small then frame image size
    {
        if (cfg->lcd_width < cfg->bg_width)
        {
            lcd_start_x = (cfg->bg_width - cfg->lcd_width) / 2;
        }
        if (cfg->lcd_height < cfg->bg_height)
        {
            lcd_start_y = (cfg->bg_height - cfg->lcd_height) / 2;
        }
    }
    //if bg data is rgb565(after hw rotate)
    dma2d_offset_blend_t dma2d_config;

    dma2d_config.pfg_addr = (char *)cfg->pfg_addr;
    dma2d_config.pbg_addr = (char *)cfg->pbg_addr;
    dma2d_config.pdst_addr = (char *)cfg->pbg_addr;
    dma2d_config.fg_color_mode = DMA2D_INPUT_ARGB8888;
    switch (cfg->bg_data_format)
    {
        case PIXEL_FMT_YUYV:
            dma2d_config.bg_color_mode = DMA2D_INPUT_YUYV;
            dma2d_config.dst_color_mode = DMA2D_OUTPUT_YUYV;
            break;
        case PIXEL_FMT_VUYY:
            dma2d_config.bg_color_mode = DMA2D_INPUT_VUYY;
            dma2d_config.dst_color_mode = DMA2D_OUTPUT_YUYV;
            break;
        case PIXEL_FMT_RGB888:
            dma2d_config.bg_color_mode = DMA2D_INPUT_RGB888;
            dma2d_config.dst_color_mode = DMA2D_OUTPUT_RGB888;
            break;
        case PIXEL_FMT_RGB565:
        default:
            dma2d_config.bg_color_mode = DMA2D_INPUT_RGB565;
            dma2d_config.dst_color_mode = DMA2D_OUTPUT_RGB565;
            break;
    }
    dma2d_config.fg_red_blue_swap = DMA2D_RB_SWAP ;
    dma2d_config.bg_red_blue_swap = DMA2D_RB_REGULAR;
    dma2d_config.dst_red_blue_swap = DMA2D_RB_REGULAR;

    dma2d_config.fg_frame_width = cfg->xsize;
    dma2d_config.fg_frame_height = cfg->ysize;
    dma2d_config.bg_frame_width = cfg->bg_width;
    dma2d_config.bg_frame_height = cfg->bg_height;
    dma2d_config.dst_frame_width = cfg->bg_width;
    dma2d_config.dst_frame_height = cfg->bg_height;

    dma2d_config.fg_frame_xpos = 0;
    dma2d_config.fg_frame_ypos = 0;
    dma2d_config.bg_frame_xpos = lcd_start_x + cfg->xpos;
    dma2d_config.bg_frame_ypos = lcd_start_y + cfg->ypos;
    dma2d_config.dst_frame_xpos = lcd_start_x + cfg->xpos;
    dma2d_config.dst_frame_ypos = lcd_start_y + cfg->ypos;

    dma2d_config.fg_pixel_byte = FOUR_BYTES;
    dma2d_config.bg_pixel_byte = TWO_BYTES;
    dma2d_config.dst_pixel_byte = TWO_BYTES;

    dma2d_config.dma2d_width = cfg->xsize;
    dma2d_config.dma2d_height = cfg->ysize;
    dma2d_config.fg_alpha_mode = DMA2D_NO_MODIF_ALPHA;
    dma2d_config.bg_alpha_mode = DMA2D_REPLACE_ALPHA;
    bk_dma2d_offset_blend(&dma2d_config);
    bk_dma2d_start_transfer();
#if (USE_DMA2D_BLEND_ISR_CALLBACKS == 1)
    if (rtos_get_semaphore(&s_blend.dma2d_complete_sem, BEKEN_NEVER_TIMEOUT) != BK_OK)
    {
        LOGE("%s, dma2d_complete_sem get failed: %d\n", __func__);
    }
#else
    while (bk_dma2d_is_transfer_busy()) {}
#endif

#endif  //CONFIG_LCD_DMA2D_BLEN
    return BK_OK;
}

#if CONFIG_BLEND
/**
 * @brief This API draw and display font such as "12:30 ÐÇÆÚÒ» sunday"
 *
 * 1: direct draw font(12:30) to yuv bg image
 * 2£ºfont rgb565 data to yuv bg image, convert bg yuv to rgb565, then rgb565 blend, and then rgb565 convert to yuv
 *                             this method performance in condition of direct yuv font not realize.
 * 3:(lcd_font->font_rotate == ROTATE_270) is the demand of lcd rotate 90 but font view still not rotate
 */
bk_err_t bk_font_blend(font_blend_cfg_t *cfg)
{
    if (blend_check_mem(cfg->xsize, cfg->ysize, cfg->font_rotate) != BK_OK)
    {
        LOGE("%s,%d\n", __func__, __LINE__);
        return BK_FAIL;
    }

    int ret = BK_OK;
    uint8_t pixel_bytes;
    uint16_t lcd_start_x = 0;
    uint16_t lcd_start_y = 0;
    register uint32_t i = 0;
    uint8_t * pbg_addr_temp = NULL;
    uint8_t *p_yuv_src = pbg_addr_temp;
    uint8_t *p_yuv_dst = s_blend.buf1.addr;
    uint8_t *p_yuv_rotate_temp = s_blend.buf2.addr;
    uint8_t *font_addr = NULL;

    if ((cfg->lcd_width < cfg->bg_width)  || (cfg->lcd_height < cfg->bg_height)) //for lcd size is small then frame image size
    {
        if (cfg->lcd_width < cfg->bg_width)
        {
            lcd_start_x = (cfg->bg_width - cfg->lcd_width) / 2;
        }
        if (cfg->lcd_height < cfg->bg_height)
        {
            lcd_start_y = (cfg->bg_height - cfg->lcd_height) / 2;
        }
    }
    if (cfg->bg_data_format == PIXEL_FMT_RGB888)
    {
        pixel_bytes = 3;
    }
    else
    {
        pixel_bytes = 2;
    }

    pbg_addr_temp = (uint8_t *)(cfg->pbg_addr + (((lcd_start_y + cfg->ypos) * cfg->bg_width) + (lcd_start_x + cfg->xpos)) * pixel_bytes);
    p_yuv_src = pbg_addr_temp;

#if 0//CONFIG_SOC_BK7258
    //  #if CONFIG_CACHE_ENABLE
    //  flush_dcache(cfg->pbg_addr, cfg->bg_height * cfg->bg_width * 2);
    //  #endif

    //  dma2d_memcpy_psram(cfg->pbg_addr, p_yuv_dst, cfg->xsize, cfg->ysize, cfg->bg_offline, 0);
    dma2d_config_t dma2d_config = {0};

    /*##-1- Configure the DMA2D Mode, Output Color Mode and output offset #############*/
    dma2d_config.init.mode         = DMA2D_M2M;             /**< Mode Memory To Memory */
    dma2d_config.init.color_mode    = DMA2D_OUTPUT_YUYV; /**< Output color mode is ARGB4444 : 16 bpp */
    dma2d_config.init.output_offset = 0;                   /**< No offset on output */
    dma2d_config.init.red_blue_swap   = DMA2D_RB_REGULAR;     /**< No R&B swap for the output image */
    dma2d_config.init.alpha_inverted = DMA2D_REGULAR_ALPHA;  /**< No alpha inversion for the output image */
    dma2d_config.init.trans_ability = TRANS_16BYTES;
    dma2d_config.init.out_byte_by_byte_reverse = NO_REVERSE;

    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].alpha_mode = DMA2D_NO_MODIF_ALPHA;      /**< Keep original Alpha from ARGB4444 input */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].input_alpha = 0xFF;                     /**< Fully opaque */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].input_color_mode = DMA2D_INPUT_YUYV; /**< Input color is ARGB4444 : 16 bpp */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].input_offset = cfg->bg_offline;                     /**< No offset in input */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].red_blue_swap   = DMA2D_RB_REGULAR;      /**< No R&B swap for the input image */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].alpha_inverted = DMA2D_REGULAR_ALPHA;   /**< No alpha inversion for the input image */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].input_data_reverse = NO_REVERSE;

    bk_dma2d_init(&dma2d_config);
    bk_dma2d_layer_config(&dma2d_config, DMA2D_FOREGROUND_LAYER);
    //  LOGD( "cfg->xsize, cfg->ysize %d, %d\n", cfg->xsize, cfg->ysize);
    bk_dma2d_transfer_config(&dma2d_config, (uint32_t)cfg->pbg_addr, (uint32_t)p_yuv_dst, cfg->xsize, cfg->ysize);
    bk_dma2d_start_transfer();
    while (bk_dma2d_is_transfer_busy()) {}
#else   //BK7256 psram can not use dma2d
    for (i = 0; i < cfg->ysize; i++)
    {
        os_memcpy((uint32_t *)p_yuv_dst, (uint32_t *)p_yuv_src, cfg->xsize * pixel_bytes);
        p_yuv_dst += (cfg->xsize * pixel_bytes);
        p_yuv_src += (cfg->bg_width * pixel_bytes);
    }
#endif
    p_yuv_dst = s_blend.buf1.addr;
    if (cfg->font_rotate == ROTATE_270)
    {
        if (PIXEL_FMT_VUYY == cfg->bg_data_format)
        {
            //step2 rotate area
            vuyy_rotate_degree90_to_yuyv((unsigned char *)p_yuv_dst, (unsigned char *)p_yuv_rotate_temp, cfg->xsize, cfg->ysize);
        }
        if (PIXEL_FMT_YUYV == cfg->bg_data_format)
        {
            yuyv_rotate_degree90_to_yuyv((unsigned char *)p_yuv_dst, (unsigned char *)p_yuv_rotate_temp, cfg->xsize, cfg->ysize);
        }
        i = cfg->xsize;
        cfg->xsize = cfg->ysize;
        cfg->ysize = i;
        cfg->font_format = FONT_YUYV;
        p_yuv_dst = s_blend.buf2.addr;
    }
    if (cfg->font_format == FONT_RGB565)  //font rgb565 data to yuv bg image
    {
        font_addr = s_blend.buf2.addr;
        if (PIXEL_FMT_VUYY == cfg->bg_data_format)
        {
            vuyy_to_rgb565_convert((unsigned char *)s_blend.buf1.addr, (unsigned char *)s_blend.buf2.addr, cfg->xsize, cfg->ysize);
        }
        else if (PIXEL_FMT_YUYV == cfg->bg_data_format)
        {
            yuyv_to_rgb565_convert((unsigned char *)s_blend.buf1.addr, (unsigned char *)s_blend.buf2.addr, cfg->xsize, cfg->ysize);
        }
        else if (PIXEL_FMT_RGB565_LE == cfg->bg_data_format)
        {
            // bg alreadr rgb565
            font_addr = s_blend.buf1.addr;
            rgb565_to_rgb565le_convert(s_blend.buf1.addr, cfg->xsize, cfg->ysize);
        }
        else if (PIXEL_FMT_RGB888 == cfg->bg_data_format)
        {
            rgb888_to_rgb565_convert(s_blend.buf1.addr, (uint16_t *)s_blend.buf2.addr, cfg->xsize, cfg->ysize);
        }
        else
        {
        }
        //  lcd_storage_capture_save("yuv_to_rgb565.rgb", s_blend.buf2.addr, len);
        font_t font;
        font.info = (ui_display_info_struct)
        {
            font_addr, 0, cfg->ysize, 0, {0}
        };
        font.width = cfg->xsize;
        font.height = cfg->ysize;
        font.font_fmt = cfg->font_format;
        for (int i = 0; i < cfg->str_num; i++)
        {
            font.digit_info = cfg->str[i].font_digit_type;
            font.s = cfg->str[i].str;
            font.font_color = cfg->str[i].font_color;
            font.x_pos = cfg->str[i].x_pos;
            font.y_pos = cfg->str[i].y_pos;
            lcd_draw_font(&font);
        }
        if (PIXEL_FMT_VUYY == cfg->bg_data_format)
        {
            rgb565_to_vuyy_convert((uint16_t *)s_blend.buf2.addr, (uint16_t *)s_blend.buf1.addr, cfg->xsize, cfg->ysize);
        }
        else if (PIXEL_FMT_YUYV == cfg->bg_data_format)
        {
            rgb565_to_yuyv_convert((uint16_t *)s_blend.buf2.addr, (uint16_t *)s_blend.buf1.addr, cfg->xsize, cfg->ysize);
        }
        else if (PIXEL_FMT_RGB565_LE == cfg->bg_data_format)
        {
            // USE this api to convert to RGB5655_LE
            rgb565_to_rgb565le_convert(s_blend.buf1.addr, cfg->xsize, cfg->ysize);
        }
        else if (PIXEL_FMT_RGB888 == cfg->bg_data_format)
        {
            rgb565_to_rgb888_convert((uint16_t *)s_blend.buf2.addr, s_blend.buf1.addr, cfg->xsize, cfg->ysize);
        }
        else
        {
        }
    }
    else  //font yuv data to yuv bg image
    {
        font_t font;
        font.info = (ui_display_info_struct)
        {
            (unsigned char *)p_yuv_dst, 0, cfg->ysize, 0, {0}
        };
        font.width = cfg->xsize;
        font.height = cfg->ysize;
        font.font_fmt = cfg->font_format;
        for (int i = 0; i < cfg->str_num; i++)
        {
            font.digit_info = cfg->str[i].font_digit_type;
            font.s = cfg->str[i].str;
            font.font_color = cfg->str[i].font_color;
            font.x_pos = cfg->str[i].x_pos;
            font.y_pos = cfg->str[i].y_pos;
            lcd_draw_font(&font);
        }
    }

    if (cfg->font_rotate == ROTATE_270)
    {
        p_yuv_rotate_temp = s_blend.buf1.addr;
        if (PIXEL_FMT_VUYY == cfg->bg_data_format)
        {
            yuyv_rotate_degree270_to_vuyy((unsigned char *)p_yuv_dst, (unsigned char *)p_yuv_rotate_temp, cfg->xsize, cfg->ysize);
        }
        else
        {
            yuyv_rotate_degree270_to_yuyv((unsigned char *)p_yuv_dst, (unsigned char *)p_yuv_rotate_temp, cfg->xsize, cfg->ysize);
        }
        // rotate back
        i = cfg->xsize;
        cfg->xsize = cfg->ysize;
        cfg->ysize = i;
    }
    p_yuv_src = s_blend.buf1.addr;
    p_yuv_dst = pbg_addr_temp;
#if 0//CONFIG_SOC_BK7258
    //      dma2d_memcpy_psram(p_yuv_src, cfg->pbg_addr, cfg->xsize, cfg->ysize, 0, cfg->bg_offline);

    /*##-1- Configure the DMA2D Mode, Output Color Mode and output offset #############*/
    dma2d_config.init.mode         = DMA2D_M2M;             /**< Mode Memory To Memory */
    dma2d_config.init.color_mode    = DMA2D_OUTPUT_YUYV; /**< Output color mode is ARGB4444 : 16 bpp */
    dma2d_config.init.output_offset = cfg->bg_offline;                   /**< No offset on output */
    dma2d_config.init.red_blue_swap   = DMA2D_RB_REGULAR;     /**< No R&B swap for the output image */
    dma2d_config.init.alpha_inverted = DMA2D_REGULAR_ALPHA;  /**< No alpha inversion for the output image */
    dma2d_config.init.trans_ability = TRANS_16BYTES;
    dma2d_config.init.out_byte_by_byte_reverse = NO_REVERSE;

    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].alpha_mode = DMA2D_NO_MODIF_ALPHA;      /**< Keep original Alpha from ARGB4444 input */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].input_alpha = 0xFF;                     /**< Fully opaque */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].input_color_mode = DMA2D_INPUT_YUYV; /**< Input color is ARGB4444 : 16 bpp */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].input_offset = 0;                     /**< No offset in input */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].red_blue_swap   = DMA2D_RB_REGULAR;      /**< No R&B swap for the input image */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].alpha_inverted = DMA2D_REGULAR_ALPHA;   /**< No alpha inversion for the input image */
    dma2d_config.layer_cfg[DMA2D_FOREGROUND_LAYER].input_data_reverse = NO_REVERSE;

    bk_dma2d_init(&dma2d_config);
    bk_dma2d_layer_config(&dma2d_config, DMA2D_FOREGROUND_LAYER);

    bk_dma2d_transfer_config(&dma2d_config, (uint32_t)p_yuv_src, (uint32_t)cfg->pbg_addr, cfg->xsize, cfg->ysize);
    bk_dma2d_start_transfer();
    while (bk_dma2d_is_transfer_busy()) {}
#else
    for (i = 0; i < cfg->ysize; i++)
    {
        os_memcpy((uint32_t *)p_yuv_dst, (uint32_t *)p_yuv_src, cfg->xsize * pixel_bytes);
        p_yuv_src += (cfg->xsize * pixel_bytes);
        p_yuv_dst += (cfg->bg_width * pixel_bytes);
    }
#endif

    return ret;
}

#endif

bk_err_t dma2d_fill_height_head_tail(frame_buffer_t *frame, media_ppi_t jpeg_ppi, media_ppi_t lcd_ppi, uint32_t color)
{
	dma2d_fill_t fill = {0};
	uint8_t fill_height = ((lcd_ppi & 0xFFFF) - (jpeg_ppi & 0xFFFF)) >> 1;

	fill.color = color;
	fill.color_format = DMA2D_OUTPUT_YUYV;
	fill.frameaddr = frame->frame;
	fill.frame_xsize = jpeg_ppi >> 16 ;
	fill.frame_ysize = jpeg_ppi & 0xFFFF;
	fill.width = jpeg_ppi >> 16;
	fill.height = fill_height;
	fill.pixel_byte =TWO_BYTES;
	fill.xpos = 0;
	fill.ypos = 0;
	dma2d_fill(&fill);
	bk_dma2d_start_transfer();

	if (rtos_get_semaphore(&s_blend.dma2d_complete_sem, BEKEN_NEVER_TIMEOUT) != BK_OK)
	{
		LOGE("%s, dma2d_complete_sem get failed: %d\n", __func__);
	}


	fill.xpos = 0;
	fill.ypos = fill_height + ( jpeg_ppi & 0xFFFF);
	dma2d_fill(&fill);
	bk_dma2d_start_transfer();
	
	LOGV("fill.frame_xsize,fill.frame_ysize, fill.width, fill.height= %d %d %d %d\n", fill.frame_xsize,fill.frame_ysize, fill.width, fill.height);

	if (rtos_get_semaphore(&s_blend.dma2d_complete_sem, BEKEN_NEVER_TIMEOUT) != BK_OK)
	{
		LOGE("%s, dma2d_complete_sem get failed: %d\n", __func__);
	}
	return 0;
}



bk_err_t blend_free_buffer(blend_t s_blend)
{
	int ret = BK_FAIL;
#if CONFIG_BLEND
	if (s_blend.buf1.addr != NULL)
	{
		os_free(s_blend.buf1.addr);
		s_blend.buf1.addr = NULL;
	}

	if (s_blend.buf2.addr != NULL)
	{
		os_free(s_blend.buf2.addr);
		s_blend.buf2.addr = NULL;
	}
	LOGD("%s lcd dma2d deinit, free buffer ok\n", __func__);
#endif
	return ret;
}

static bk_err_t blend_malloc_buffer(blend_t s_blend)
{
	int ret = BK_FAIL;
#if CONFIG_BLEND
	if (s_blend.buf1.addr == NULL)
	{
#if BLEND_MALLOC_SRAM
		s_blend.buf1.addr = (uint8_t *)os_malloc(LCD_BLEND_MALLOC_SIZE);
        s_blend.buf1.size = LCD_BLEND_MALLOC_SIZE;
        s_blend.buf1.is_malloc_psram = 0;
		if (s_blend.buf1.addr == NULL)
			LOGE("%s, blend malloc buf1 NULL\r\n", __func__);
		else
			LOGD("%s, blend malloc sram (default) buf1 = %p, size = %d \r\n", __func__, s_blend.buf1.addr, s_blend.buf1.size);
#endif
	}

	if (s_blend.buf2.addr == NULL)
	{
#if BLEND_MALLOC_SRAM
		s_blend.buf2.addr = (uint8_t *)os_malloc(LCD_BLEND_MALLOC_RGB_SIZE);
        s_blend.buf2.size = LCD_BLEND_MALLOC_RGB_SIZE;
        s_blend.buf1.is_malloc_psram = 0;
		if (s_blend.buf2.addr == NULL)
			LOGE("%s, blend malloc buf2 NULL\r\n", __func__);
		else
			LOGD("%s, blend malloc sram (default) buf2= %p, size = %d\r\n", __func__, s_blend.buf2.addr, s_blend.buf2.size);
#endif
	}

	if (s_blend.buf1.addr != NULL && s_blend.buf2.addr != NULL)
	{
		ret =  BK_OK;
	}
	else
	{
#if BLEND_MALLOC_SRAM
		if (s_blend.buf1.addr != NULL)
		{
			os_free(s_blend.buf1.addr);
			s_blend.buf1.addr = NULL;
            s_blend.buf1.size = 0;
		}

		if (s_blend.buf2.addr != NULL)
		{
			os_free(s_blend.buf2.addr);
			s_blend.buf2.addr = NULL;
            s_blend.buf2.size = 0;
		}
#endif
		ret =  BK_ERR_NO_MEM;
	}
    LOGE("blend malloc %p %p\r\n", s_blend.buf1.addr, s_blend.buf2.addr);
#endif
	return ret;
}

bk_err_t bk_blend_init(void)
{
	bk_err_t ret = BK_OK;
    if (s_blend.state != false)
    {
        LOGD("%s, already init\n", __func__);
        return ret;
    }
    s_blend.state = true;

#if (CONFIG_LCD_DMA2D_BLEND)
	bk_dma2d_driver_init();
#if (USE_DMA2D_BLEND_ISR_CALLBACKS == 1)
	ret = rtos_init_semaphore_ex(&s_blend.dma2d_complete_sem, 1, 0);
	if (ret != BK_OK)
	{
		LOGE("%s dma2d_sem init failed: %d\n", __func__, ret);
		return ret;
	}
	ret = rtos_init_semaphore_ex(&s_blend.dma2d_err_sem, 1, 0);
	if (ret != BK_OK)
	{
		LOGE("%s dma2d_err_sem init failed: %d\n", __func__, ret);
		return ret;
	}
	bk_dma2d_int_enable(DMA2D_CFG_ERROR | DMA2D_TRANS_ERROR | DMA2D_TRANS_COMPLETE,1);
	bk_dma2d_register_int_callback_isr(DMA2D_CFG_ERROR_ISR, dma2d_config_error);
	bk_dma2d_register_int_callback_isr(DMA2D_TRANS_ERROR_ISR, dma2d_transfer_error);
	bk_dma2d_register_int_callback_isr(DMA2D_TRANS_COMPLETE_ISR, dma2d_transfer_complete);
#endif 
#endif
    
    blend_malloc_buffer(s_blend);

	return ret;
}


bk_err_t bk_blend_deinit(void)
{
	bk_err_t ret = BK_OK;
    if (s_blend.state == false)
    {
        LOGD("%s, already deinit\n", __func__);
        return ret;
    }
#if CONFIG_LCD_DMA2D_BLEND
	bk_dma2d_driver_deinit();
#if USE_DMA2D_BLEND_ISR_CALLBACKS
	ret = rtos_deinit_semaphore(&s_blend.dma2d_complete_sem);
	if (ret != BK_OK)
	{
	LOGE("%s dma2d_complete_sem deinit failed: %d\n", __func__, ret);
	return ret;
	}
	ret = rtos_deinit_semaphore(&s_blend.dma2d_err_sem);
	if (ret != BK_OK)
	{
	LOGE("%s dma2d_err_sem deinit failed: %d\n", __func__, ret);
	return ret;
	}
#endif
#endif
    blend_free_buffer(s_blend);
    s_blend.state = false;
    LOGD("%s, deinit complete\n", __func__);

 	return ret;
}


