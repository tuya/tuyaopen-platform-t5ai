...
typedef struct bk_route_hook_t
{
    struct netif * netif;
    struct raw_pcb * pcb;
    struct bk_route_hook_t * next;
} bk_route_hook_t;
...
static bk_route_hook_t * s_hooks;
...
static void ra_recv_handler(struct netif * netif, const uint8_t * icmp_payload, uint16_t payload_len, const ip6_addr_t * src_addr)
{
    ...
    while (payload_len >= 2)
    {
            ...
            payload_len >= opt_len)
        {
            ...
            os_memcpy(&rio_header, icmp_payload, sizeof(rio_header));
            ...
            LWIP_LOGD("Received RIO\n");
            if (rio_data_len >= prefix_len_bytes)
            {
                ip6_addr_t prefix;
                bk_route_entry_t route;

                os_memset(&prefix, 0, sizeof(prefix));
                os_memcpy(&prefix.addr, rio_data, prefix_len_bytes);
                route.netif            = netif;
                ...
                LWIP_LOGD("prefix %s lifetime %lu\n", ip6addr_ntoa(&prefix), route.lifetime_seconds);
                if (bk_route_table_add_route_entry(&route) == NULL)
                {
                    LWIP_LOGD("Failed to add route table entry\n");
                }
                else
                {
                    LWIP_LOGD("Added entry to route table\n");
                }
            }
        }
...
static uint8_t icmp6_raw_recv_handler(void * arg, struct raw_pcb * pcb, struct pbuf * p, const ip_addr_t * addr)
{
    ...
    ip6_addr_t src;
    ip6_addr_t dest;
    bk_route_hook_t * hook = (bk_route_hook_t *) arg;

    os_memcpy(src.addr, ip6_header->src.addr, sizeof(src.addr));
    os_memcpy(dest.addr, ip6_header->dest.addr, sizeof(dest.addr));
#if LWIP_IPV6_SCOPES
    src.zone = 0;
#endif
    ...
    if (p->tot_len != p->len)
    {
        LWIP_LOGD("Ignore segmented ICMP packet\n");
        return 0;
    }
    if (p->tot_len <= sizeof(struct ip6_hdr) + sizeof(struct icmp6_hdr))
    {
        LWIP_LOGD("Ignore invalid ICMP packet\n");
        return 0;
    }
...
int8_t bk_route_hook_init()
{
    struct netif * lwip_netif = &netif_list[0];
    ip_addr_t router_group    = IPADDR6_INIT_HOST(0xFF020000, 0, 0, 0x02);
    bk_route_hook_t * hook = NULL;
    uint8_t ret               = 0;

    if (lwip_netif == NULL)
    {
        LWIP_LOGD("Invalid network interface\n");
        return -1;
    }

    for (bk_route_hook_t * iter = s_hooks; iter != NULL; iter++)
    {
        if (iter->netif == lwip_netif)
        {
            LWIP_LOGD("Hook already installed on netif, skip...\n");
            return 0;
        }
    }

    hook = (bk_route_hook_t *)os_malloc(sizeof(bk_route_hook_t));
    if (hook == NULL)
    {
        LWIP_LOGD("Cannot allocate hook\n");
        return -1;
    }

    if (mld6_joingroup_netif(lwip_netif, ip_2_ip6(&router_group)) != ERR_OK)
    {
        LWIP_LOGD("Failed to join multicast group\n");
        ret = -1;
        goto exit;
    }
