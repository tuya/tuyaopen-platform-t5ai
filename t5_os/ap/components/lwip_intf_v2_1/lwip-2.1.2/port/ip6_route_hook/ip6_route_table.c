...
static bk_route_entry_t s_route_entries[MAX_RIO_ROUTE];

static bk_route_entry_t * find_route_entry(const bk_route_entry_t * route_entry)
{
...
static bk_route_entry_t * find_empty_route_entry(void)
{
    for (size_t i = 0; i < LWIP_ARRAYSIZE(s_route_entries); i++)
    {
        if (s_route_entries[i].netif == NULL)
...
{
    bk_route_entry_t * route = (bk_route_entry_t *) arg;

    bk_route_table_remove_route_entry(route);
}

bk_route_entry_t * bk_route_table_add_route_entry(const bk_route_entry_t * route_entry)
{
    ...
    bk_route_entry_t * entry = find_route_entry(route_entry);
...
int8_t bk_route_table_remove_route_entry(bk_route_entry_t * route_entry)
{
    ...
    for (bk_route_entry_t * moved = route_entry; moved < &s_route_entries[LWIP_ARRAYSIZE(s_route_entries) - 1]; moved++)
    {
        *moved = *(moved + 1);
        if (moved->netif == NULL)
        {
...
static inline bool route_match(const bk_route_entry_t * route, const ip6_addr_t * dest)
{
