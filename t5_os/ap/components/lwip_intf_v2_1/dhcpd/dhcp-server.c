...
int dhcp_server_init(void *intrfc_handle)
{
	...
	if (setsockopt(dhcps.sock, SOL_SOCKET, SO_BINDTODEVICE, &ifreq_name, sizeof(struct ifreq)) != 0) {
	...
	if (dhcp_nack_dns_server_handler) {
		...
		if (dhcps.dnssock < 0)
			return -1;
		if (setsockopt(dhcps.dnssock, SOL_SOCKET, SO_BINDTODEVICE, &ifreq_name, sizeof(struct ifreq)) != 0) {
			dhcp_e("failed to set SO_BINDTODEVICE\r\n");
			return -1;
		}
	}
