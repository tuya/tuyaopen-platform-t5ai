...
#define GT911_XY_CORDINATE_SWAP_EN   (0)
#define GT911_XY_CORDINATE_ROTATE_EN (0)
...
int gt911_init(const tp_i2c_callback_t *cb, tp_sensor_user_config_t *config)
{
	...
	// xy cordinate swap.
	#if (GT911_XY_CORDINATE_SWAP_EN > 0)
		cfg_table[GT911_MODULE_SWITCH1_POS] &= 0xF7;
		cfg_table[GT911_MODULE_SWITCH1_POS] |= 0x08;
	#endif
...
void gt911_touch_down(void *buf, int8_t id, int16_t x, int16_t y, int16_t w)
{
	...
	#if (GT911_XY_CORDINATE_ROTATE_EN > 0)
		// special rotate process start
		int16_t sw = ppi_to_pixel_x(tp_sensor_gt911.def_ppi); // tp screen width.
		int16_t sh = ppi_to_pixel_y(tp_sensor_gt911.def_ppi); // tp screen high.
