...
static bk_err_t test_flash_count_time(volatile uint32_t start_addr, uint32_t len, uint32_t test_times, uint32_t mode,uint32_t print_cnt){
	uint32_t tmp;
	u8 buf[256];
	uint32_t addr = start_addr;
	uint32_t length = len;
...
static bk_err_t test_flash_count_time(volatile uint32_t start_addr, uint32_t len, uint32_t test_times, uint32_t mode,uint32_t print_cnt)
{
	...
	BK_DUMP_OUT("----- FLASH COUNT TIME TEST BEGIN -----\r\n");
	BK_DUMP_OUT("Start address:0x08%x\r\n", start_addr);
	BK_DUMP_OUT("Test length:%d Bytes\r\n", len);
	BK_DUMP_OUT("Test times:%d \r\n", test_times);
	BK_DUMP_OUT("Test mode:%d \r\n", mode);
	BK_DUMP_OUT("===============================\r\n");

	for(int i = 0; i <= test_times; i++) {
		if(mode) {
			// mode!=0: need protect none, read, earse, write, protect all in each loop
			// mode=0: need read, earse, write in each loop, not protect
			...
			if(i % print_cnt == 0) {
				BK_DUMP_OUT("[dis security %d time] >>>>> cost time: %d us.\r\n", i, tick_cnt);
			}
		}
		...
		// 	bk_flash_erase_sector(addr);
		// }
		for (addr = start_addr; addr < tmp; addr += 0x10000) {//64K
...
static void flash_command_test(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "U") == 0) {
		...
		uint32_t mode = 0;
		if(argc >= 6)
			mode = os_strtoul(argv[5], NULL, 16);
		uint32_t print_cnt = PRINT_CNT;
		if(argc >= 7)
			print_cnt = os_strtoul(argv[6], NULL, 10);
		test_flash_count_time(addr, len, test_times, mode, print_cnt);
		return;
	}
