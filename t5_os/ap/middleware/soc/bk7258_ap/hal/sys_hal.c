...
bk_err_t sys_hal_switch_cpu_bus_freq(pm_cpu_freq_e cpu_bus_freq)
{
    ...
    switch(cpu_bus_freq)
    {
		...
		case PM_CPU_FRQ_240M://cpu0:120m;cpu1:240m;bus:120m
		    sys_hal_ctrl_vdddig_h_vol(0xC);//0.8V
			ret = sys_hal_core_bus_clock_ctrl(0x3,0x1,0x0,0x0,0x1);
			break;
...
int32 sys_hal_core_int_group2_disable(uint32 core_id, uint32 param)
{
	...
	if(CPU0_CORE_ID == core_id) {
		sys_hal_int_group2_disable(param);
	}
	else if(CPU1_CORE_ID == core_id) {
		reg = sys_ll_get_cpu1_int_32_63_en_value();
		value = reg;
...
int32 sys_hal_core_int_group2_enable(uint32 core_id, uint32 param)
{
	...
	if(CPU0_CORE_ID == core_id) {
		sys_hal_int_group2_enable(param);
	}
	else if(CPU1_CORE_ID == core_id) {
		reg = sys_ll_get_cpu1_int_32_63_en_value();
		reg |= (param);
...
void sys_hal_set_saradc_config(void)
{
	...
	sys_ll_set_ana_reg2_gadc_nobuf_enable(1);
	sys_ll_set_ana_reg2_gadc_refbuf_ictrl(2);
	sys_ll_set_ana_reg2_gadc_inbuf_ictrl(2);
	sys_ll_set_ana_reg2_gadc_cmp_ictrl(2);
	// sys_ll_set_ana_reg2_xtalh_ctune(30);
}
...
void sys_hal_early_init(void)
{
	...
	sys_hal_analog_set(ANALOG_REG9, 0x787BC34A); //tenglong20231017: revert <15> to 1 for softstart, should not change it
