...
#define QSPI_LL_REG_BASE(_qspi_unit_id)            (SOC_QSPI0_REG_BASE + (0x20000 * _qspi_unit_id))
#define QSPI_DATA_LL_REG_BASE(_qspi_unit_id)       (SOC_QSPI0_DATA_BASE + (0x4000000 * _qspi_unit_id))
...
#define QSPI1_LL_CLK_PIN     GPIO_2
#define QSPI1_LL_CSN_PIN     GPIO_3
#define QSPI1_LL_IO0_PIN     GPIO_4
#define QSPI1_LL_IO1_PIN     GPIO_5
#define QSPI1_LL_IO2_PIN     GPIO_6
#define QSPI1_LL_IO3_PIN     GPIO_7
...
static inline void qspi_ll_init_command(qspi_hw_t *hw, const qspi_cmd_t *cmd)
{
    ...
    uint8_t addr_hh; // Modified by TUYA
	...
	if(INDIRECT_MODE == cmd->work_mode) {
		if(QSPI_WRITE == cmd->op) {
			...
			if(FLASH_WR_S0_S7_CMD == ((cmd->cmd) & QSPI_F_CMD1_M) || (FLASH_WR_S8_S15_CMD == ((cmd->cmd) & QSPI_F_CMD1_M))) {
                ...
                hw->cmd_c_h.v &= (~(QSPI_F_CMD2_M << QSPI_F_CMD2_S));
				hw->cmd_c_h.v |= ((addr_l & QSPI_F_CMD2_M) << QSPI_F_CMD2_S);
                hw->cmd_c_cfg1.v = 0x30;

            } else if (cmd->addr_valid_bit == QSPI_ADDR_VALID_BIT16) {
			...
			if((FLASH_RD_S0_S7_CMD == cmd->cmd) || (FLASH_RD_S8_S15_CMD == cmd->cmd) ||
				...
				hw->cmd_d_cfg1.v = 0xc;
				if (cmd->dummy_cycle) {
					hw->cmd_d_cfg2.dummy_mode = 4; // Modified by TUYA
				} else {
                ...
                if(QSPI_4WIRE == cmd->wire_mode) {
					hw->cmd_d_cfg1.v = 0xe8;
				} else {
					hw->cmd_d_cfg1.v = 0xc0;
				}
				if (cmd->dummy_cycle) {
					hw->cmd_d_cfg2.dummy_mode = 3; // Modified by TUYA
				} else {
					hw->cmd_d_cfg2.dummy_mode = 0;
				}
            }  else if (cmd->addr_valid_bit == QSPI_ADDR_VALID_BIT32) {
