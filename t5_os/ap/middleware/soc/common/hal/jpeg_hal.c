...
bk_err_t jpeg_hal_switch_mode(jpeg_hal_t *hal, const jpeg_config_t *config)
{
	...
	if (config->vsync == SYNC_LOW_LEVEL)
	{
	...
	if (config->hsync == SYNC_LOW_LEVEL)
	{
...
bk_err_t jpeg_hal_enable_clk(jpeg_hal_t *hal, uint32_t value)
{
	...
	jpeg_ll_enable_jpeg_mode(hal->hw);
	return BK_OK;
}
...
bk_err_t jpeg_hal_start_common(jpeg_hal_t *hal, yuv_mode_t mode)
{
	if (mode == YUV_MODE)
	{
		jpeg_ll_disable_jpeg_mode(hal->hw);
		jpeg_ll_enable_yuv_mode(hal->hw);
	}
	else
	{
		jpeg_ll_disable_yuv_mode(hal->hw);
		jpeg_ll_enable_jpeg_mode(hal->hw);
	}
...
bk_err_t jpeg_hal_enable_bitrate_ctrl(jpeg_hal_t *hal, uint8_t enable)
{
	...
	{
		jpeg_ll_enable_bitrate_ctrl(hal->hw);
		jpeg_ll_set_bitrate_step(hal->hw, 1);
	}
	else
	{
		jpeg_ll_disable_bitrate_ctrl(hal->hw);
	}
