		...
		case 0xC0:	/* SOF0 (baseline JPEG) */
			...
			for (i = 0; i < 3; i++)
			{
				...
				{	/* Y component */
					...
					if (b == 0x21)
					{
						jpg_dec_st.jpeg_enc_mode = JPEG_ENCODE_YUV422;
					}
					else if (b == 0x22)
					{
						jpg_dec_st.jpeg_enc_mode = JPEG_ENCODE_YUV420;
					}
					else if (b == 0x11)
					{
						jpg_dec_st.jpeg_enc_mode = JPEG_ENCODE_YUV444;
					}
					else
					{
						jpg_dec_st.jpeg_enc_mode = JPEG_ENCODE_UNSTD;
					}
...
static uint32_t jpg_dec_by_line_get_dec_lines(void)
{
	uint32_t line = 0;
#if CONFIG_SOC_BK7236XX
	line = (8 * (1 + jpeg_dec_ll_get_reg0x52_jpeg_line_num()));
#endif
	return line;
}
...
static int jpg_dec_config(uint16_t xpixel, uint16_t ypixel, uint32_t length, unsigned char *input_buf, unsigned char * output_buf)
{
	...
	if (jpeg_dec_hal_get_jpeg_dec_linen())
		jpeg_dec_ll_set_reg0x5b_value(xpixel * jpg_dec_by_line_get_dec_lines() * 2 * 2);  //ping-pong mem to save decode data
	else
		jpeg_dec_ll_set_reg0x5b_value(xpixel * ypixel * 2);  //WR len
...
//    jpeg_dec_ll_set_reg0x5e_dec_ffd9_int(auto_int_en);
    jpeg_dec_ll_set_reg0x50_ffd9_check(auto_int_en);
#endif
#endif
}
...
#if CONFIG_JPEGDEC_HW_SUPPORT_FFD9_CHECK
//    jpeg_dec_ll_set_reg0x5e_dec_ffd9_int(auto_int_en);
	jpeg_dec_ll_set_reg0x50_ffd9_check(line_int_en);
#endif
#endif
}
...
{
#if CONFIG_SOC_BK7256XX
		jpeg_dec_ll_set_reg0x2_jpeg_line_num(line_num);  //UVC 640*480 LINE24 int 480 / ((2+1)*8) = 480/24=20 times int every 24 line decode
#elif CONFIG_SOC_BK7236XX
		jpeg_dec_ll_set_reg0x52_jpeg_line_num(line_num);  //UVC 640*480 LINE24 int 480 / ((2+1)*8) = 480/24=20 times int every 24 line decode
#endif
}



static void jpeg_dec_out_format(pixel_format_t   pixel_fmt)
{
#if CONFIG_SOC_BK7236XX
...
JRESULT jpeg_dec_hal_get_img_info(uint32_t frame_size, uint8_t *input_buf, jpeg_dec_res_t *result)
{
	...
	jpg_dec_st.jpg_file_size = frame_size;
	ret = jd_prepare(&jdec, jpeg_dec_input_func, jpg_dec_st.workbuf, WORK_AREA_SIZE, NULL);

	if (ret == JDR_OK) {
		result->pixel_x = jdec.width;		/* Image size to output */
...
//		BK_LOGD(NULL, "jd prepare error return %x \r\n", ret);
    ...
    jpeg_dec_ll_set_reg0x56_len_huf_dcy(bits_num);
    ...
    jpeg_dec_ll_set_reg0x56_len_huf_dcuv(bits_num);
    ...
    jpeg_dec_ll_set_reg0x56_len_huf_acy(bits_num);
    ...
    jpeg_dec_ll_set_reg0x56_len_huf_acuv(bits_num);
...
JRESULT jd_decomp_hw(void)
{
	...
	//jpeg_dec_out_format(PIXEL_FMT_VUYY);  //jpeg decode out YUYV by default
	...
	rc = JDR_OK;
	jpeg_dec_ll_set_reg0x8_dec_cmd(JPEGDEC_DC_START);
	return rc;
}
