...
#if (CONFIG_SOC_SMP || CONFIG_SOC_BK7239_SMP_TEMP)
...
#include "../../../include/components/shell_task.h"
#endif
...
{
	spin_unlock(&dump_spin_lock);  // free spin lock before releasing log port.
	                               // so other core can get spinlock to print registers before reset by this core.
	// #if (CONFIG_SHELL_ASYNCLOG)
...
static void user_except_handler(uint32_t reset_reason, SAVED_CONTEXT *regs)
{
    ...
    if (0 == g_enter_exception) {
		...
		spin_lock(&dump_spin_lock);  // get spin lock again.
#endif
