...
void ui_event_tabview_screen(lv_event_t * e)
{
    ...
    if(event_code == LV_EVENT_GESTURE &&  lv_indev_get_gesture_dir(lv_indev_get_act()) == LV_DIR_BOTTOM) 
    {
        if(1 == g_annim_flag)
        {
            bk_printf("[%s][%d] del annim when page ver change\r\n", __FUNCTION__, __LINE__);
    ...
    if(event_code == LV_EVENT_SCREEN_LOADED) {
        bk_printf("[%s][%d] play annim when page load\r\n", __FUNCTION__, __LINE__);
...
static void get_tab_act_index_event_handler(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED) {
        if (obj)
        {
            ...
            if(tab_id != 0)
            {
                bk_printf("[%s][%d] del annim when page hor change\r\n", __FUNCTION__, __LINE__);
                lv_anim_del(NULL, (lv_anim_exec_xcb_t)_ui_anim_callback_set_x);
                g_annim_flag = 0;
            }
            else
            {
                if(0 == g_annim_flag)
                {
                    bk_printf("[%s][%d] play annim when page hor change to 1\r\n", __FUNCTION__, __LINE__);
                    g_annim_flag = 1;
