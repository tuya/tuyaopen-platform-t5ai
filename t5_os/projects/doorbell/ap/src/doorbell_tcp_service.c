...
int doorbell_tcp_img_send_packet(uint8_t *data, uint32_t len)
{
    ...
    return doorbell_socket_sendto(&db_tcp_service->img_fd, (struct sockaddr *)&db_tcp_service->img_remote, data, len, -((int)sizeof(db_trans_head_t)));
}
...
int doorbell_tcp_aud_send_packet(uint8_t *data, uint32_t len)
{
    ...
    return doorbell_socket_sendto(&db_tcp_service->aud_fd, (struct sockaddr *)&db_tcp_service->aud_remote, data, len, -((int)sizeof(db_trans_head_t)));
}
...
static media_transfer_cb_t doorbell_tcp_img_channel =
{
...
void doorbell_tcp_service_init(void)
{
    ...
    db_tcp_service->img_channel = doorbell_transmission_malloc(0, DOORBELL_TCP_NETWORK_MAX_SIZE);
    db_tcp_service->aud_channel = doorbell_transmission_malloc(DOORBELL_TCP_NETWORK_MAX_SIZE, DOORBELL_TCP_NETWORK_MAX_SIZE);
